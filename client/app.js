...
const phoneLayoutSessionId = /^[A-Za-z0-9_-]{12,40}$/.test(appearanceParams.get('phone-layout') || '') ? appearanceParams.get('phone-layout') : '';
...
const INTERFACE_STYLE_KEY = 'fivefold-arc:interface-style';
...
const dom = {
  ...
  activeSeatBar: $('#activeSeatBar'), gameMenu: $('#gameMenu'), moreButton: $('#moreButton'), tableSkinMenuField: $('#tableSkinMenuField'), tableSkinMenu: $('#tableSkinMenu'), viewCelebrationButton: $('#viewCelebrationButton'), saveGameButton: $('#saveGameButton'), saveGameDeck: $('#saveGameDeck'),
...
function recordClientDiagnostic(kind) {
  if (appearancePreviewMode || !state?.podCode || transport.localMode) return;
  const heap = performance.memory?.usedJSHeapSize;
  void transport.recordClientDiagnostic({ kind, build: DIAGNOSTIC_BUILD, viewport: diagnosticViewport(), deviceMemoryGb: diagnosticDeviceMemory(), visibility: document.visibilityState === 'hidden' ? 'hidden' : 'visible', jsHeapMb: Number.isFinite(heap) ? Math.round(heap / 1_048_576) : null }).catch(() => {});
}
const accountChoice = $('#accountChoice'); const playActions = $('#playActions'); const signInButton = $('#signInButton'); const continueGuestButton = $('#continueGuestButton'); const myGamesButton = $('#myGamesButton'); const myGamesDialog = $('#myGamesDialog'); const myGamesStatus = $('#myGamesStatus'); const myGamesContent = $('#myGamesContent'); const myGamesSignInButton = $('#myGamesSignInButton'); const emailSignInFields = $('#emailSignInFields'); const emailSignInButton = $('#emailSignInButton'); const forgotPasswordButton = $('#forgotPasswordButton'); const createAccountButton = $('#createAccountButton'); const exportAccountButton = $('#exportAccountButton'); const deleteAccountButton = $('#deleteAccountButton'); const signOutButton = $('#signOutButton'); const accountEmail = $('#accountEmail'); const accountPassword = $('#accountPassword'); const accountPreferredNameField = $('#accountPreferredNameField'); const accountPreferredName = $('#accountPreferredName'); const accountDefaultPlayerCountField = $('#accountDefaultPlayerCountField'); const accountDefaultPlayerCount = $('#accountDefaultPlayerCount'); const accountDefaultRoundLimitField = $('#accountDefaultRoundLimitField'); const accountDefaultRoundLimit = $('#accountDefaultRoundLimit'); const saveAccountPreferencesButton = $('#saveAccountPreferencesButton'); const myDecksButton = $('#myDecksButton'); const myDecksDialog = $('#myDecksDialog'); const myDecksStatus = $('#myDecksStatus'); const myDecksContent = $('#myDecksContent'); const myDecksSignInButton = $('#myDecksSignInButton'); const createDeckButton = $('#createDeckButton'); const deckCommanderName = $('#deckCommanderName'); const deckCommanderNameTwo = $('#deckCommanderNameTwo'); const deckCommanderNameTwoField = $('#deckCommanderNameTwoField'); const deckName = $('#deckName'); const deckNotes = $('#deckNotes'); const deckFavorite = $('#deckFavorite');
const baseSealMarkup = $('.counter-sigil')?.innerHTML || ''; const baseModeNavMarkup = new Map($$('[data-mode]').map(button => [button.dataset.mode, button.innerHTML]));
const accountSignedInStatus = $('#accountSignedInStatus');
const accountInterfaceStyleField = $('#accountInterfaceStyleField'); const accountInterfaceStyle = $('#accountInterfaceStyle'); const accountPersonalSkinField = $('#accountPersonalSkinField'); const accountPersonalSkin = $('#accountPersonalSkin'); const accountUsePersonalSkinField = $('#accountUsePersonalSkinField'); const accountUsePersonalSkin = $('#accountUsePersonalSkin');
let state = null;
...
function turnSound() { try { const sound = localStorage.getItem('fivefold-arc:turn-sound'); return ['ding', 'chime', 'bell', 'two-note', 'tap'].includes(sound) ? sound : 'ding'; } catch { return 'ding'; } }
function turnSoundVolume() { try { const volume = Number(localStorage.getItem('fivefold-arc:turn-sound-volume')); return Number.isFinite(volume) && volume >= 10 && volume <= 100 ? volume : 55; } catch { return 55; } }
function setTurnSound(sound, volume) { try { localStorage.setItem('fivefold-arc:turn-sound', sound); localStorage.setItem('fivefold-arc:turn-sound-volume', String(volume)); } catch { /* preference is optional */ } }
...
function playTurnCue(preview = false) {
  ...
  if (!preview && (mode === 'vibrate' || mode === 'both')) try { navigator.vibrate?.([45, 35, 70]); } catch { /* unsupported, including iPhone */ }
  if (preview || mode === 'sound' || mode === 'both') try {
    prepareTurnCueAudio(preview); if (!turnCueAudio || turnCueAudio.state === 'suspended') return; const oscillator = turnCueAudio.createOscillator(); const gain = turnCueAudio.createGain();
    const now = turnCueAudio.currentTime; const profile = turnSound(); const volume = turnSoundVolume() / 100 * .22; const notes = { ding: [[660, 0, .16]], chime: [[523, 0, .13], [784, .09, .22]], bell: [[880, 0, .26]], 'two-note': [[523, 0, .1], [784, .13, .16]], tap: [[220, 0, .06]] }[profile];
    oscillator.connect(gain).connect(turnCueAudio.destination); notes.forEach(([frequency, start, duration]) => { oscillator.frequency.setValueAtTime(frequency, now + start); gain.gain.setValueAtTime(.0001, now + start); gain.gain.exponentialRampToValueAtTime(volume, now + start + .01); gain.gain.exponentialRampToValueAtTime(.0001, now + start + duration); }); oscillator.start(); oscillator.stop(now + Math.max(...notes.map(note => note[1] + note[2])) + .02);
...
function showAchievementUnlocks(gameKey) {
  const unlocked = unlockedAchievementsByGame.get(gameKey) || [];
  if (!unlocked.length || dom.achievementDialog.open) return false;
  dom.achievementDialog.dataset.gameKey = gameKey;
  dom.achievementTitle.textContent = unlocked.length === 1 ? 'Achievement uncovered' : `${unlocked.length} achievements uncovered`;
  dom.achievementSummary.textContent = unlocked.length === 1 ? 'Your game added something new to your chronicle.' : 'Your game uncovered several new entries in your chronicle.';
  dom.achievementUnlockList.innerHTML = unlocked.map(achievement => { const rarity = achievement.rarity || 'common'; return `<article class="achievement-card" data-rarity="${escapeHtml(rarity)}"><span aria-hidden="true">${achievementRarityMark(rarity)}</span><div><small class="achievement-rarity">${escapeHtml(achievementRarityLabel(rarity))}</small><strong>${escapeHtml(achievement.title)}</strong><small>${escapeHtml(achievement.detail)}</small></div></article>`; }).join('');
  dom.achievementDialog.showModal(); return true;
}
async function loadSaveGameDecks() {
  try { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/decks', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return; const { decks } = await response.json(); dom.saveGameDeck.innerHTML = `<option value="">No deck selected</option>${decks.map(deck => `<option value="${escapeHtml(deck.deckId)}">${escapeHtml([deck.commanderName, deck.name].filter(Boolean).join(' · '))}</option>`).join('')}`; } catch { /* Saving a game remains available without a deck. */ }
}
async function loadCommanderDecks() {
  try { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/decks', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return; const { decks } = await response.json(); dom.commanderDeck.innerHTML = `<option value="">No deck selected</option>${decks.map(deck => `<option value="${escapeHtml(deck.deckId)}">${escapeHtml([deck.commanderName, deck.name].filter(Boolean).join(' · '))}</option>`).join('')}`; dom.commanderDeck.value = selectedDeckId; dom.commanderDeckField.hidden = false; } catch { /* Deck choice remains optional. */ }
}
function sortSetupDecks(decks) { return [...decks].sort((left, right) => Number(right.favorite) - Number(left.favorite) || String(left.name || left.commanderName || '').localeCompare(String(right.name || right.commanderName || ''), undefined, { sensitivity: 'base' })); }
async function loadSetupDecks() {
  try { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/decks', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return; const { decks } = await response.json(); setupDecks = sortSetupDecks(decks); const options = `<option value="">No deck selected</option>${setupDecks.map(deck => `<option value="${escapeHtml(deck.deckId)}">${escapeHtml(`${deck.favorite ? '★ ' : ''}${[deck.commanderName, deck.name, deck.colors?.join('')].filter(Boolean).join(' · ')}`)}</option>`).join('')}`; [dom.createDeck, dom.joinDeck].forEach(select => { select.innerHTML = options; select.value = ''; }); $('#joinSignInDeckButton').hidden = true; updateFormatSetup(); } catch {} }
function renderVictory() {
  ...
  dom.saveGameButton.hidden = !result || !winner || state?.localSimulation;
  dom.saveGameDeck.hidden = true;
...
function openQuickFeedback() { if (!state?.gameResult || state.localSimulation || state.feedbackPromptSeatId !== transport.seatId || localStorage.getItem('fivefold-arc:quick-feedback-until') > Date.now()) return; $('#quickFeedbackDialog').showModal(); }
function setInterfaceStyle(style, { persistAccount = false } = {}) {
  interfaceStyle = style === 'dial' ? 'dial' : 'button';
...
const publishedSkinStyle = document.createElement('style'); publishedSkinStyle.textContent = 'html.game-skin-active body{background:radial-gradient(circle at var(--skin-atmosphere-position,50% 0%),color-mix(in srgb,var(--skin-atmosphere,#000) calc(var(--skin-atmosphere-strength,0) * 100%),transparent),transparent 58%),var(--bg);background-size:cover;background-position:center}html.game-skin-active #gameView .pod-seat{border-radius:var(--skin-seat-radius,8px);border-color:color-mix(in srgb,#5b594f calc(var(--skin-seat-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--skin-seat-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--skin-seat-blur,0) * 1px))}html.game-skin-active #gameView[data-appearance-seat="flat"] .pod-seat{border-radius:0;box-shadow:none}html.game-skin-active #gameView[data-appearance-seat="glass"] .pod-seat{background:color-mix(in srgb,var(--surface) calc(var(--skin-seat-opacity,100) * .72%),transparent)}html.game-skin-active #gameView .end-turn-button{border-radius:var(--skin-button-radius,8px);border-color:color-mix(in srgb,var(--brass) calc(var(--skin-button-border,70) * 1%),transparent);background:color-mix(in srgb,var(--brass) calc(var(--skin-button-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--skin-button-blur,0) * 1px))}html.game-skin-active #gameView[data-appearance-button="outline"] .end-turn-button{background:transparent;color:var(--brass)}html.game-skin-active #gameView .pod-strip{translate:var(--skin-pod-strip-x,0) var(--skin-pod-strip-y,0);scale:var(--skin-pod-strip-scale,1);transform-origin:center}html.game-skin-active #gameView .counter-readout{translate:var(--skin-counter-readout-x,0) var(--skin-counter-readout-y,0);scale:var(--skin-counter-readout-scale,1);transform-origin:center}html.game-skin-active #gameView .dial-gesture{translate:var(--skin-dial-surface-x,0) var(--skin-dial-surface-y,0);scale:var(--skin-dial-surface-scale,1);transform-origin:center}html.game-skin-active #gameView .source-panel{translate:var(--skin-commander-damage-x,0) var(--skin-commander-damage-y,0);scale:var(--skin-commander-damage-scale,1);transform-origin:center}html.game-skin-active #gameView #turnActions{translate:var(--skin-turn-actions-x,0) var(--skin-turn-actions-y,0);scale:var(--skin-turn-actions-scale,1);transform-origin:center}html.game-skin-active #gameView .adjust-controls{translate:var(--skin-adjust-controls-x,0) var(--skin-adjust-controls-y,0);scale:var(--skin-adjust-controls-scale,1);transform-origin:center}html.game-skin-active #gameView .custom-life-button{translate:var(--skin-custom-life-x,0) var(--skin-custom-life-y,0);scale:var(--skin-custom-life-scale,1);transform-origin:center}html.game-skin-active #gameView .commander-tax-quick{translate:var(--skin-commander-tax-x,0) var(--skin-commander-tax-y,0);scale:var(--skin-commander-tax-scale,1);transform-origin:center}html.game-skin-active #gameView .mode-nav{translate:var(--skin-mode-nav-x,0) var(--skin-mode-nav-y,0);scale:var(--skin-mode-nav-scale,1);transform-origin:center}'; document.head.append(publishedSkinStyle);
const publishedSkinArtStyle = document.createElement('style'); publishedSkinArtStyle.textContent = 'html.game-skin-active .mode-nav img{width:22px;height:22px;object-fit:contain}html.game-skin-active .counter-sigil img{width:100%;height:100%;object-fit:contain}html.game-skin-active .skin-overlay{position:absolute;inset:0;z-index:2;width:100%;height:100%;object-fit:cover;pointer-events:none;mix-blend-mode:var(--skin-overlay-blend,normal);opacity:var(--skin-overlay-opacity,.3);transform:scale(var(--skin-overlay-scale,1));transform-origin:center}html.game-skin-active .skin-dial-art{border-radius:50%;transform:scale(var(--skin-overlay-scale,1)) rotate(var(--dial-seal-rotation,0deg))}html.game-skin-active .pod-seat{position:relative;overflow:hidden}html.game-skin-active .pod-seat>.seat-name,html.game-skin-active .pod-seat>.seat-life,html.game-skin-active .dial-controls>*:not(.skin-overlay){position:relative;z-index:3}'; document.head.append(publishedSkinArtStyle);
async function loadPublishedSkinChoices(selectedId = '') { try { const response = await fetch('/api/game-skins', { cache: 'no-store' }); if (!response.ok) return; publishedSkins = (await response.json()).skins || []; const options = publishedSkins.map(skin => `<option value="${escapeHtml(skin.id)}">${escapeHtml(skin.name)}</option>`).join(''); accountPersonalSkin.innerHTML = `<option value="">Standard shared appearance</option>${options}`; accountPersonalSkin.value = publishedSkins.some(skin => skin.id === selectedId) ? selectedId : ''; const tableSkin = $('#tableSkin'); if (tableSkin) tableSkin.innerHTML = `<option value="">Standard appearance</option>${options}`; } catch { /* A skin catalog outage never blocks play. */ } }
function clearPublishedSkin() { const root = document.documentElement; root.classList.remove('game-skin-active'); root.removeAttribute('data-game-skin'); ['--bg','--surface','--surface-2','--ink','--muted','--brass','--brass-dark'].forEach(name => root.style.removeProperty(name)); document.body.style.backgroundImage = ''; document.body.style.backgroundSize = ''; document.body.style.backgroundPosition = ''; dom.game.removeAttribute('data-appearance-seat'); dom.game.removeAttribute('data-appearance-button'); document.querySelectorAll('.skin-overlay').forEach(node => node.remove()); const seal = $('.counter-sigil'); if (seal) seal.innerHTML = baseSealMarkup; baseModeNavMarkup.forEach((markup, mode) => { const button = $(`[data-mode="${mode}"]`); if (button) button.innerHTML = markup; }); }
function applyPublishedSkin(skin) { const root = document.documentElement; root.classList.add('game-skin-active'); root.dataset.gameSkin = skin.id; root.style.setProperty('--bg', skin.background || '#080808'); root.style.setProperty('--surface', skin.surface || '#161616'); root.style.setProperty('--surface-2', skin.surface || '#161616'); root.style.setProperty('--ink', skin.text || '#f4ecd9'); root.style.setProperty('--muted', skin.muted || '#afa590'); root.style.setProperty('--brass', skin.accent || '#d8ae61'); root.style.setProperty('--brass-dark', skin.secondary || '#6e552a'); root.style.setProperty('--skin-atmosphere', skin.atmosphereColor || skin.secondary || '#000'); root.style.setProperty('--skin-atmosphere-strength', String(Math.min(0.8, Math.max(0, Number(skin.atmosphereStrength || 0) / 100)))); root.style.setProperty('--skin-atmosphere-position', skin.atmospherePosition === 'bottom' ? '50% 100%' : skin.atmospherePosition === 'center' ? '50% 50%' : '50% 0%'); root.style.setProperty('--skin-seat-radius', `${Math.min(24, Math.max(0, Number(skin.seatRadius) || 8))}px`); ['seat','button'].forEach(prefix => { root.style.setProperty(`--skin-${prefix}-opacity`, String(Math.min(100, Math.max(35, Number(skin[`${prefix}Opacity`]) || 100)))); root.style.setProperty(`--skin-${prefix}-blur`, String(Math.min(16, Math.max(0, Number(skin[`${prefix}Blur`]) || 0)))); root.style.setProperty(`--skin-${prefix}-border`, String(Math.min(100, Math.max(0, Number(skin[`${prefix}Border`]) || 70)))); }); root.style.setProperty('--skin-button-radius', `${Math.min(24, Math.max(0, Number(skin.buttonRadius) || 8))}px`); const layout = skin.modeLayouts?.[interfaceStyle] || skin.componentLayout || {}; const regions = { podStrip: 'pod-strip', counterReadout: 'counter-readout', dialSurface: 'dial-surface', commanderDamage: 'commander-damage', turnActions: 'turn-actions', adjustControls: 'adjust-controls', customLife: 'custom-life', commanderTax: 'commander-tax', modeNav: 'mode-nav' }; Object.entries(regions).forEach(([key, css]) => { const placement = layout.componentLayout?.[key] || {}; root.style.setProperty(`--skin-${css}-x`, `${Math.min(100, Math.max(-100, Number(placement.x) || 0))}px`); root.style.setProperty(`--skin-${css}-y`, `${Math.min(140, Math.max(-140, Number(placement.y) || 0))}px`); root.style.setProperty(`--skin-${css}-scale`, String(Math.min(140, Math.max(70, Number(placement.scale) || 100)) / 100)); }); dom.game.dataset.appearanceSeat = skin.seatTreatment || 'raised'; dom.game.dataset.appearanceButton = skin.buttonTreatment || 'solid'; if (skin.backgroundData) { document.body.style.backgroundImage = `linear-gradient(rgb(0 0 0 / .32), rgb(0 0 0 / .32)), url("${skin.backgroundData}")`; document.body.style.backgroundSize = `${Math.min(200, Math.max(50, Number(skin.backgroundScale) || 100))}% auto`; document.body.style.backgroundPosition = `${Math.min(100, Math.max(0, Number(skin.backgroundPositionX) || 50))}% ${Math.min(100, Math.max(0, Number(skin.backgroundPositionY) || 50))}%`; } }
function applyPublishedSkinArt(skin) { document.querySelectorAll('.skin-overlay').forEach(node => node.remove()); const seal = $('.counter-sigil'); if (seal && skin.sealData) { seal.replaceChildren(); const image = new Image(); image.alt = ''; image.src = skin.sealData; seal.append(image); } Object.entries(skin.symbolData || {}).forEach(([mode, data]) => { const button = $(`[data-mode="${mode}"]`); if (!button || !data) return; button.replaceChildren(); const image = new Image(); image.alt = ''; image.src = data; image.style.transform = `scale(${Math.min(2, Math.max(.6, Number(skin.symbolScale?.[mode]) || 100) / 100)})`; const label = document.createElement('small'); label.textContent = mode === 'commander' ? 'Cmdr' : mode; button.append(image, label); }); const add = (target, data, slot) => { if (!target || !data) return; const image = new Image(); image.className = `skin-overlay${slot === 'dial' && skin.dialArtSpin ? ' skin-dial-art' : ''}`; image.alt = ''; image.src = data; image.style.setProperty('--skin-overlay-opacity', String(Math.min(70, Math.max(0, Number(skin.overlayOpacity?.[slot]) || 0)) / 100)); image.style.setProperty('--skin-overlay-scale', String(Math.min(1.6, Math.max(.5, Number(skin.overlayScale?.[slot]) || 100) / 100))); image.style.setProperty('--skin-overlay-blend', skin.overlayBlend?.[slot] || 'normal'); target.append(image); }; document.querySelectorAll('.pod-seat').forEach(seat => add(seat, skin.overlayData?.seat, 'seat')); add($('.counter-stage'), skin.overlayData?.counter, 'counter'); add($('.dial-gesture'), skin.overlayData?.dial, 'dial'); }
async function applySelectedAppearance() { const shared = state?.tableSkinId || ''; const personal = accountPreferences?.usePersonalSkin ? accountPreferences.personalSkinId : ''; const id = personal || shared; if (!id) { activeGameSkinId = ''; clearPublishedSkin(); return; } if (activeGameSkinId === id) return; try { const response = await fetch(`/api/game-skins/${encodeURIComponent(id)}`, { cache: 'no-store' }); if (!response.ok) throw new Error(); const skin = (await response.json()).skin; activeGameSkinId = id; applyPublishedSkin(skin); if (state) render(); applyPublishedSkinArt(skin); } catch { activeGameSkinId = ''; clearPublishedSkin(); } }
function applyAccountPreferences(preferences) {
  ...
  accountPreferences = preferences; accountUsePersonalSkin.checked = Boolean(preferences.usePersonalSkin); void loadPublishedSkinChoices(preferences.personalSkinId || '');
...
async function showMyGames() {
  ...
  try {
    const token = await currentAccountToken();
    if (!token) { myGamesDialog.close(); openSignIn(); return; }
    ...
    const history = (await response.json()).history; enterApp(true); void loadSetupDecks(); myGamesSignInButton.hidden = true; emailSignInButton.hidden = true; emailSignInFields.hidden = true; accountPassword.value = ''; forgotPasswordButton.hidden = true; createAccountButton.hidden = true; exportAccountButton.hidden = false; deleteAccountButton.hidden = false; signOutButton.hidden = false; accountPreferredNameField.hidden = false; saveAccountPreferencesButton.hidden = false;
    const preferencesResponse = await fetch('/api/account/preferences', { headers: { authorization: `Bearer ${token}` } }); const preferences = preferencesResponse.ok ? (await preferencesResponse.json()).preferences : { preferredName: null, defaultPlayerCount: 4, defaultRoundLimitMinutes: null, interfaceStyle: 'button', personalSkinId: null, usePersonalSkin: false }; applyAccountPreferences(preferences); accountDefaultPlayerCountField.hidden = false; accountDefaultRoundLimitField.hidden = false; accountInterfaceStyleField.hidden = false; accountPersonalSkinField.hidden = false; accountUsePersonalSkinField.hidden = false; if (preferences.preferredName) { dom.createName.value ||= preferences.preferredName; dom.joinName.value ||= preferences.preferredName; } const playerDefault = $(`input[name="playerCount"][value="${preferences.defaultPlayerCount || 4}"]`); if (playerDefault) playerDefault.checked = true; dom.roundLimitMinutes.value ||= preferences.defaultRoundLimitMinutes || '';
    ...
    const profileSettings = [accountDefaultPlayerCountField, accountDefaultRoundLimitField, accountInterfaceStyleField, accountPersonalSkinField, accountUsePersonalSkinField, saveAccountPreferencesButton, exportAccountButton, deleteAccountButton, signOutButton];
    const overviewProfileFields = [accountPreferredNameField];
    profileSettings.forEach(item => { item.hidden = true; }); overviewProfileFields.forEach(item => { item.hidden = false; });
    profileSections.forEach((section, index) => { section.dataset.profilePanel = String(index); section.hidden = index !== 0; });
    const deckStatsIndex = profileSections.findIndex(section => section.querySelector('h3')?.textContent === 'Deck statistics');
    const gameHistoryIndex = profileSections.findIndex(section => section.querySelector('h3')?.textContent === 'Game history');
    const profileTabs = [{ label: 'Overview', panel: 0 }, { label: 'Achievements', panel: 1 }, ...(deckStatsIndex >= 0 ? [{ label: 'Deck stats', panel: deckStatsIndex }] : []), ...(gameHistoryIndex >= 0 ? [{ label: 'Games', panel: gameHistoryIndex }] : []), { label: 'Settings', panel: 'settings' }];
    myGamesContent.insertAdjacentHTML('afterbegin', `<nav class="profile-tabs" aria-label="Profile sections">${profileTabs.map((tab, index) => `<button type="button" data-profile-tab="${tab.panel}"${index ? '' : ' aria-current="page"'}>${tab.label}</button>`).join('')}</nav>`);
    profileSections[1]?.querySelector('h3')?.replaceChildren('Achievements');
    myGamesContent.querySelectorAll('[data-profile-tab]').forEach(button => button.addEventListener('click', () => { const active = button.dataset.profileTab; profileSections.forEach((section, index) => { section.hidden = String(index) !== active; }); overviewProfileFields.forEach(item => { item.hidden = active !== '0'; }); profileSettings.forEach(item => { item.hidden = active !== 'settings'; }); myGamesContent.querySelectorAll('[data-profile-tab]').forEach(tab => tab.toggleAttribute('aria-current', tab === button)); }));
    $('#gameHistorySearch')?.addEventListener('input', event => { const query = event.currentTarget.value.trim().toLowerCase(); $$('#gameHistoryList [data-history-game]').forEach(item => { item.hidden = Boolean(query) && !item.textContent.toLowerCase().includes(query); }); });
...
async function showMyDecks(signIn = false) {
  myDecksContent.hidden = true; myDecksStatus.textContent = signIn ? 'Signing in…' : 'Loading your decks…';
  try {
    ...
    const response = await fetch('/api/account/decks?archived=all', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('Your decks are not available yet.');
    const decks = (await response.json()).decks; savedDecks = decks; enterApp(true); myDecksStatus.textContent = decks.length ? `${decks.length} saved deck${decks.length === 1 ? '' : 's'}` : 'No saved decks yet.'; myDecksContent.innerHTML = decks.length ? `<ul>${decks.map(deck => `<li><strong>${deck.favorite ? '★ ' : ''}${escapeHtml(deck.commanderName)}</strong>${deck.archived ? ' <small>Archived</small>' : ''}${deck.name ? `<br><small>${escapeHtml(deck.name)}</small>` : ''}${deck.colors?.length ? `<br><small>${escapeHtml(deck.colors.join(' '))}</small>` : ''}${deck.notes ? `<br><small>${escapeHtml(deck.notes)}</small>` : ''}<br><button type="button" class="text-action" data-edit-deck="${escapeHtml(deck.deckId)}">Edit</button> <button type="button" class="text-action" data-toggle-favorite="${escapeHtml(deck.deckId)}">${deck.favorite ? 'Unfavorite' : 'Favorite'}</button> <button type="button" class="text-action" data-toggle-archive="${escapeHtml(deck.deckId)}">${deck.archived ? 'Restore' : 'Archive'}</button> <button type="button" class="text-action" data-export-deck="${escapeHtml(deck.deckId)}">Export deck</button> <button type="button" class="text-action" data-remove-deck="${escapeHtml(deck.deckId)}">Remove</button></li>`).join('')}</ul>` : ''; myDecksContent.hidden = !decks.length; myDecksSignInButton.hidden = true; createDeckButton.disabled = false;
...
function beginDeckEdit(deckId) { const deck = savedDecks.find(item => item.deckId === deckId); if (!deck) return; editingDeckId = deckId; const names = deck.commanderNames || [deck.commanderName]; $(`input[name="deckCommanderCount"][value="${names.length}"]`).checked = true; deckCommanderName.value = names[0] || ''; deckCommanderNameTwo.value = names[1] || ''; deckCommanderNameTwoField.hidden = names.length !== 2; deckName.value = deck.name || ''; deckNotes.value = deck.notes || ''; deckFavorite.checked = deck.favorite === true; $$('input[name="deckColors"]').forEach(input => { input.checked = deck.colors?.includes(input.value); }); createDeckButton.textContent = 'Update deck'; myDecksStatus.textContent = `Editing ${deck.commanderName}. Make your correction, then choose Update deck.`; }
async function createDeck() {
  const token = await currentAccountToken(); const commanderNames = [deckCommanderName.value, deckCommanderNameTwo.value].map(name => name.trim()).filter(Boolean); if (!token || !commanderNames.length || commanderNames.length !== Number($('input[name="deckCommanderCount"]:checked')?.value || 1)) return;
  createDeckButton.disabled = true;
  try { const colors = $$('input[name="deckColors"]:checked').map(input => input.value); const body = { commanderNames, name: deckName.value, colors, notes: deckNotes.value, favorite: deckFavorite.checked }; const response = await fetch(editingDeckId ? `/api/account/decks/${encodeURIComponent(editingDeckId)}` : '/api/account/decks', { method: editingDeckId ? 'PATCH' : 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify(body) }); if (!response.ok) throw new Error('This deck could not be saved yet.'); editingDeckId = null; createDeckButton.textContent = 'Save deck'; deckCommanderName.value = ''; deckCommanderNameTwo.value = ''; deckName.value = ''; deckNotes.value = ''; deckFavorite.checked = false; $$('input[name="deckColors"]').forEach(input => { input.checked = false; }); await showMyDecks(); void loadSetupDecks(); } catch (error) { myDecksStatus.textContent = error?.message || 'This deck could not be saved yet.'; createDeckButton.disabled = false; }
}
async function saveAccountPreferences() { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/preferences', { method: 'PUT', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ preferredName: accountPreferredName.value, defaultPlayerCount: Number(accountDefaultPlayerCount.value), defaultRoundLimitMinutes: accountDefaultRoundLimit.value || null, interfaceStyle, personalSkinId: accountPersonalSkin.value || null, usePersonalSkin: accountUsePersonalSkin.checked }) }); if (!response.ok) { myGamesStatus.textContent = 'Your preferences could not be saved yet.'; return; } accountPreferences = (await response.json()).preferences; void applySelectedAppearance(); dom.createName.value ||= accountPreferredName.value.trim(); dom.joinName.value ||= accountPreferredName.value.trim(); myGamesStatus.textContent = 'Preferences saved.'; }
async function removeAccountItem(kind, id) {
  const token = await currentAccountToken(); if (!token) return;
  const response = await fetch(`/api/account/${kind}/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('That item could not be removed.');
  if (kind === 'games') await showMyGames(); else await showMyDecks();
}
async function updateSavedDeck(deckId, changes) { const token = await currentAccountToken(); if (!token) return; const response = await fetch(`/api/account/decks/${encodeURIComponent(deckId)}`, { method: 'PATCH', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify(changes) }); if (!response.ok) throw new Error('That deck could not be updated.'); await showMyDecks(); void loadSetupDecks(); }
async function exportDeckSummary(deckId) { const token = await currentAccountToken(); if (!token) return; const [decksResponse, historyResponse] = await Promise.all([fetch('/api/account/decks?archived=all', { headers: { authorization: `Bearer ${token}` } }), fetch('/api/account/history', { headers: { authorization: `Bearer ${token}` } })]); if (!decksResponse.ok || !historyResponse.ok) throw new Error('That deck summary is not available yet.'); const deck = (await decksResponse.json()).decks.find(item => item.deckId === deckId); const history = (await historyResponse.json()).history; if (!deck) return; const games = history.games.filter(game => game.deckId === deckId); const wins = games.filter(game => game.won).length; const summary = { exportedAt: Date.now(), deck: { commanderNames: deck.commanderNames, name: deck.name, colors: deck.colors, notes: deck.notes }, results: { gamesPlayed: games.length, wins, winRate: games.length ? wins / games.length : null, placements: games.map(game => game.place).filter(Boolean) } }; const url = URL.createObjectURL(new Blob([JSON.stringify(summary, null, 2)], { type: 'application/json' })); const link = document.createElement('a'); link.href = url; link.download = `${(deck.name || deck.commanderName).replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '') || 'fivefold-arc-deck'}-summary.json`; link.click(); URL.revokeObjectURL(url); }
async function exportAccountData() {
  const token = await currentAccountToken(); if (!token) return;
  const response = await fetch('/api/account/export', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('Your data export is not available yet.');
  const url = URL.createObjectURL(new Blob([JSON.stringify((await response.json()).export, null, 2)], { type: 'application/json' })); const link = document.createElement('a'); link.href = url; link.download = 'fivefold-arc-my-data.json'; link.click(); URL.revokeObjectURL(url);
}

async function deleteAccountData() {
  if (!window.confirm('Permanently delete your saved games, decks, and account data? This cannot be undone.')) return;
  const token = await currentAccountToken(); if (!token) throw new Error('Sign in to delete your account data.');
...
async function signOut() {
  ...
  try {
    ...
    selectedDeckId = ''; setupDecks = []; savedDecks = [];
    automaticallySavedGameKey = null; automaticSaveInFlightKey = null;
    enterApp(false);
    if (myGamesDialog.open) myGamesDialog.close(); if (myDecksDialog.open) myDecksDialog.close();
  } catch (error) {
    myGamesStatus.textContent = error?.message || 'Sign-out did not finish. Please try again.';
...
function saveTable() {
  if (!state || state.localSimulation || !/^[A-Z0-9]{6}$/.test(state.podCode)) return;
  const entry = { code: state.podCode, seatId: transport.seatId, name: displayName(state.players.find(player => player.id === state.ownerPlayerId)), isHost: transport.seatId === state.hostSeatId, lastUsedAt: Date.now() };
  try { localStorage.setItem(SAVED_TABLES_KEY, JSON.stringify([entry, ...savedTables().filter(table => table.code !== entry.code || table.seatId !== entry.seatId)].slice(0, 8))); } catch { /* Storage is optional; reclaim remains available when the browser can retain it. */ }
}
function latestRecoverableHostPodCode() {
  const recoverableCodes = new Set(transport.hostRecoveryPodCodes());
  return savedTables()
...
async function saveGameToHistory({ automatic = false, gameKey = victoryKey(state?.gameResult) } = {}) {
  ...
  try {
    ...
    const { counterTotals, achievementFacts } = await transport.getPersonalMatchMoment();
    const response = await fetch('/api/account/games', { method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ tableSize: state.players.filter(player => player.connectionStatus !== 'waiting').length, won: result.winnerSeatId === seatId, place: place || null, outcomeDescription: result.declarationDetail || null, commanderName: you.commanderNames?.filter(Boolean).join(' / ') || null, deckId: selectedDeckId || null, counterTotals, achievementFacts, sourceGameId: `${state.podCode}:${result.decidedAt}:${seatId}` }) });
...
function renderSavedTables() {
  ...
  dom.savedTables.hidden = tables.length === 0;
  dom.savedTablesList.innerHTML = tables.map(table => `<button class="saved-table-button" type="button" data-saved-code="${escapeHtml(table.code)}"><span><strong>Pod ${escapeHtml(table.code)}</strong><br><span>${escapeHtml(table.name || `P${Number(table.seatId) + 1}`)} · ${table.isHost ? 'host' : `P${Number(table.seatId) + 1}`}</span></span><em>Return</em></button>`).join('');
  dom.savedTablesList.querySelectorAll('[data-saved-code]').forEach(button => button.addEventListener('click', () => openJoinCode(button.dataset.savedCode)));
}
async function openJoinCode(code) {
...
function openJoinQr() {
  if (!state || state.localSimulation) return;
  const code = state.podCode;
  dom.joinQrImage.src = `${apiBaseFromPage()}/api/rooms/${encodeURIComponent(code)}/join-qr.svg`;
  dom.joinQrCode.textContent = `POD ${code}`;
  if (!dom.joinQrDialog.open) dom.joinQrDialog.showModal();
}
async function copyJoinLink(button) {
  try {
    ...
    showError(new Error('Copy is not available here. Scan the QR code or use the pod code shown above.'));
  }
}
...
function selectedGameFormat() { return $('input[name="gameFormat"]:checked')?.value || 'commander'; }
function updateFormatSetup() { const format = selectedGameFormat(); const commander = format === 'commander'; dom.createCommanderNames.previousElementSibling.hidden = !commander; dom.createCommanderNames.hidden = !commander; dom.createDeckField.hidden = !commander || !setupDecks.length; $('#customStartingLifeField').hidden = format !== 'custom'; $('#createStepTwoNext').textContent = commander ? 'Next: commander' : 'Next: table settings'; $('#gameFormatHelp').textContent = commander ? '40 life with commander damage and commander tax.' : format === 'casual' ? '20 life by default. Commander damage and commander tax are off.' : 'Choose the life total and counters your table wants. Commander-only rules are off.'; const joiningCommander = joinGameFormat === 'commander'; dom.joinCommanderNames.previousElementSibling.hidden = !joiningCommander; dom.joinCommanderNames.hidden = !joiningCommander; dom.joinDeckField.hidden = !joiningCommander || !setupDecks.length; }
...
function renderCommanderNameFields(container, count, names = [], identities = []) {
  ...
  container.querySelectorAll('[data-lookup-commander]').forEach(button => button.addEventListener('click', async () => { const field = button.closest('.commander-identity-field'); const input = field.querySelector('input'); const status = field.querySelector('.commander-lookup-status'); const name = input.value.trim(); if (!name) { input.focus(); return; } button.disabled = true; status.textContent = 'Finding commander…'; try { const card = await lookupCommanderIdentity(name); input.value = card.name; input.dataset.commanderColors = card.colors.join(','); delete input.dataset.commanderLookupFailed; field.querySelector('.commander-manual-colors')?.remove(); status.textContent = `✓ Confirmed: ${card.colors.join('') || 'Colorless'}`; } catch (error) { delete input.dataset.commanderColors; input.dataset.commanderLookupFailed = 'true'; status.textContent = error.message; if (container === dom.createCommanderNames && !field.querySelector('.commander-manual-colors')) { field.insertAdjacentHTML('beforeend', '<div class="commander-manual-colors"><button type="button" class="secondary-action" data-enter-colors>Enter colors manually</button><button type="button" class="text-action" data-skip-colors>Keep without colors</button></div>'); field.querySelector('[data-enter-colors]').addEventListener('click', () => { const chooser = document.createElement('fieldset'); chooser.className = 'commander-color-chooser'; chooser.innerHTML = '<legend>Color identity</legend>W U B R G'.split(' ').map(color => `<label><input type="checkbox" value="${color}">${color}</label>`).join(''); chooser.addEventListener('change', () => { input.dataset.commanderColors = [...chooser.querySelectorAll('input:checked')].map(item => item.value).join(','); status.textContent = `Manual identity: ${input.dataset.commanderColors || 'Colorless'}`; }); field.querySelector('.commander-manual-colors').replaceWith(chooser); }); field.querySelector('[data-skip-colors]').addEventListener('click', () => { status.textContent = 'Kept without a color identity.'; field.querySelector('.commander-manual-colors').remove(); }); } } finally { button.disabled = false; } }));
}
...
function showSharedGame(snapshot) { state = stateFromSnapshot(snapshot); saveTable(); renderSavedTables(); showView(dom.game); render(); void applySelectedAppearance(); }
...
function renderTurnFlow() {
  ...
  promptRadiationAfterHandoff(isOwnerActive, handoff);
  clearInterval(turnTicker);
  if (!appearancePreviewMode) turnTicker = setInterval(() => { if (state && !dom.game.hidden) renderTurnFlow(); }, 1000);
}
function radiationTurnKey(handoff) { return handoff ? `${state.podCode}:${handoff.toSeatId}:${handoff.handedOffAt}` : ''; }
function promptRadiationAfterHandoff(isOwnerActive, handoff) {
  const ownerId = state.localSimulation ? state.activePlayerId : state.ownerPlayerId;
  if (!isOwnerActive || !handoff || handoff.toSeatId !== Number(ownerId.slice(1)) - 1) return;
  const key = radiationTurnKey(handoff); const player = state.players.find(item => item.id === ownerId); if (!key || key === resolvedRadiationTurnKey || !player || player.radiation <= 0 || dom.radiationDialog.open) return;
  dom.radiationNonlandCount.value = '0'; dom.radiationNonlandCount.max = String(player.radiation); dom.radiationDetail.textContent = `You have ${player.radiation} radiation counter${player.radiation === 1 ? '' : 's'}. Mill that many cards, then enter how many were nonlands.`; dom.radiationDialog.showModal(); dom.radiationNonlandCount.focus();
}
async function resolveRadiation() {
  const nonlandCount = Number(dom.radiationNonlandCount.value); const handoff = state?.turn?.lastHandoff; const key = radiationTurnKey(handoff); const ownerId = state?.localSimulation ? state.activePlayerId : state?.ownerPlayerId; const player = state?.players.find(item => item.id === ownerId);
  if (!Number.isInteger(nonlandCount) || nonlandCount < 0 || !player || nonlandCount > player.radiation || !key) { dom.radiationNonlandCount.focus(); return; }
  dom.resolveRadiationButton.disabled = true;
  try {
    if (transport.status === 'local') { const removed = Math.min(nonlandCount, player.radiation); player.radiation -= removed; player.life -= nonlandCount; showLifeChange(player.id, -nonlandCount, player.life + nonlandCount, player.life); }
    else { const result = await transport.resolveRadiation(nonlandCount); if (result.conflict) throw new Error('The table changed first. Check your radiation counters and try again.'); }
    resolvedRadiationTurnKey = key; dom.radiationDialog.close('confirm'); render();
  } catch (error) { showError(error); } finally { dom.resolveRadiationButton.disabled = false; }
}
function render() {
  renderDeveloperTools();
  dom.game.dataset.interfaceStyle = interfaceStyle;
  const commanderFormat = state.gameFormat === 'commander';
  if (!commanderFormat && state.mode === 'commander') state.mode = 'life';
  $$('[data-mode="commander"]').forEach(button => { button.hidden = !commanderFormat; });
  dom.commanderTaxQuickButton.closest('.commander-tax-quick').hidden = !commanderFormat;
  dom.commanderSetupButton.hidden = !commanderFormat;
  ...
  if (tableHost) { dom.tableSkinMenu.innerHTML = `<option value="">Standard appearance</option>${publishedSkins.map(skin => `<option value="${escapeHtml(skin.id)}">${escapeHtml(skin.name)}</option>`).join('')}`; dom.tableSkinMenu.value = state.tableSkinId || ''; dom.tableSkinMenu.disabled = !state.localSimulation && transport.status !== 'connected'; }
  ...
  dom.markDevelopmentButton.hidden = state.localSimulation || transport.seatId !== state.hostSeatId; dom.markDevelopmentButton.disabled = transport.status !== 'connected'; dom.markDevelopmentButton.textContent = state.sessionKind === 'development' ? 'Mark standard table' : 'Mark development run';
...
function renderJoinSeatClaim() {
  if (!dom.joinSeatClaim) return;
  const seat = dom.joinSeat.value;
  dom.joinSeatClaim.textContent = /^P[1-8]$/.test(seat)
    ? `This phone will control only ${seat}. Everyone else can view its confirmed state, but cannot change its counters.`
    : 'Choose an open seat to see what this phone will control.';
}
...
async function handoffTurn() {
  ...
  if (transport.status === 'local') {
    state.players.forEach(evaluatePlayer); const fromSeatId = Number(state.turnSeatId.slice(1)) - 1; const living = state.players.filter(player => player.connectionStatus !== 'waiting' && !player.eliminated); const sequence = living.length ? living : state.players.filter(player => player.connectionStatus !== 'waiting'); const currentIndex = sequence.findIndex(player => player.id === state.turnSeatId); const toSeatId = Number(sequence[(currentIndex + 1) % sequence.length].id.slice(1)) - 1; const handedOffAt = Date.now();
...
function openResetDialog({ nextGame = false } = {}) {
  dom.resetTitle.textContent = nextGame ? 'Start next game?' : 'Reset this game?';
  dom.resetDetail.textContent = nextGame ? 'Counters and elimination markers return to zero. Your pod, seats, and commander setup stay the same.' : 'Life returns to the starting total. All other counters and elimination markers return to zero. Pod setup stays the same.';
  dom.confirmResetButton.textContent = nextGame ? 'Start next game' : 'Reset game';
...
function openTurnSoundDialog() { dom.gameMenu.hidden = true; dom.moreButton.setAttribute('aria-expanded', 'false'); dom.turnSoundChoice.value = turnSound(); dom.turnSoundVolume.value = String(turnSoundVolume()); dom.turnSoundVolumeValue.value = `${turnSoundVolume()}%`; dom.turnSoundDialog.showModal(); }
...
async function openCardCamera() {
  dom.gameMenu.hidden = true; dom.moreButton.setAttribute('aria-expanded', 'false');
  dom.cardLookupStatus.textContent = ''; dom.cardLookupResult.hidden = true; dom.cardLookupResult.replaceChildren(); dom.cardCameraDialog.showModal(); dom.firstCardTitle.focus();
}
function renderCardLookup(card) {
  const retrieved = new Date(card.retrievedAt).toLocaleString();
  const faces = card.faces?.length ? `<details><summary>Show card faces</summary>${card.faces.map(face => `<p><strong>${escapeHtml(face.name)}</strong>${face.typeLine ? ` · ${escapeHtml(face.typeLine)}` : ''}<br>${escapeHtml(face.oracleText || 'No Oracle rules text is listed for this face.').replace(/\n/g, '<br>')}</p>`).join('')}</details>` : '';
...
function renderInteractionAdvice(advice, first, second) { return `<section class="interaction-advice"><h3>Best-effort rules explanation</h3><p class="field-help"><strong>Result:</strong> ${escapeHtml(advice.result)}</p><p class="field-help"><strong>Assumptions:</strong> ${escapeHtml(advice.assumptions)}</p><p><strong>How it works:</strong></p><ol>${advice.steps.map(step => `<li>${escapeHtml(step)}</li>`).join('')}</ol><p><strong>Why:</strong> ${escapeHtml(advice.why)}</p><p class="field-help">${escapeHtml(advice.limitations)}</p><p class="field-help">To clarify: ${escapeHtml(advice.questions.join(' '))}</p><p class="field-help"><strong>Sources:</strong> ${first.gathererUrl ? `<a href="${escapeHtml(first.gathererUrl)}" target="_blank" rel="noreferrer">${escapeHtml(first.name)} on Gatherer</a>` : escapeHtml(first.name)}${second.gathererUrl ? ` · <a href="${escapeHtml(second.gathererUrl)}" target="_blank" rel="noreferrer">${escapeHtml(second.name)} on Gatherer</a>` : ` · ${escapeHtml(second.name)}`} · <a href="https://magic.wizards.com/en/rules" target="_blank" rel="noreferrer">Comprehensive Rules</a></p></section>`; }
function renderCommunityCombos(result) {
  ...
  if (!result.combos?.length) return `<section class="interaction-advice"><h3>Established-interaction lookup</h3><p class="field-help">No established interaction was found in the Commander Spellbook database. That does not prove the cards have no interaction. A clearly labeled best-effort explanation follows.</p><p class="field-help">Community catalogue checked ${escapeHtml(retrieved)}: <a href="${escapeHtml(result.sourceUrl)}" target="_blank" rel="noreferrer">Commander Spellbook</a>.</p></section>`;
...
async function lookupCardTitles() {
  const firstName = dom.firstCardTitle.value.trim(); const secondName = dom.secondCardTitle.value.trim(); const situation = dom.interactionSituation.value.trim();
  if (!firstName) { dom.firstCardTitle.focus(); return; }
  dom.cardLookupStatus.textContent = 'Loading current Oracle text…';
  try { const [first, second] = await Promise.all([lookupCard(firstName), secondName ? lookupCard(secondName) : null]); const comboResult = second ? await lookupCardInteraction(first.name, second.name).catch(() => ({ unavailable: true, combos: [], retrievedAt: new Date().toISOString() })) : null; const noEstablishedInteraction = second && !comboResult.unavailable && !comboResult.combos?.length; dom.firstCardTitle.value = first.name; if (second) dom.secondCardTitle.value = second.name; dom.cardLookupResult.innerHTML = `${renderCardLookup(first)}${second ? renderCardLookup(second) + renderCommunityCombos(comboResult) + (noEstablishedInteraction ? renderInteractionAdvice(createInteractionAdvice(first, second, situation), first, second) : '') : ''}`; dom.cardLookupResult.hidden = false; dom.cardLookupStatus.textContent = second ? (noEstablishedInteraction ? 'Current Oracle text and rulings are shown. No established database interaction was found, so a best-effort explanation is included.' : 'Current Oracle text, published rulings, and established interaction results are shown.') : 'Current Oracle text and published rulings are shown.'; }
...
fillSetupControls(); refreshSetupCommanderNames(); updateFormatSetup(); renderDeveloperTools(); renderConnection(); renderSavedTables();
let createStep = 1;
function showCreateStep(step) { createStep = step; $$('[data-create-step]').forEach(section => { section.hidden = Number(section.dataset.createStep) !== step; }); window.scrollTo({ top: 0, behavior: 'instant' }); }
let joinStep = 2;
function showJoinStep(step) { joinStep = step; $$('[data-join-step]').forEach(section => { section.hidden = Number(section.dataset.joinStep) !== step; }); window.scrollTo({ top: 0, behavior: 'instant' }); }
function skipCreateCommanders() { dom.createCommanderNames.querySelectorAll('input[name^="commanderName"]').forEach(input => { input.value = ''; delete input.dataset.commanderColors; }); showCreateStep(4); }
...
$('#createForm').addEventListener('submit', async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const gameFormat = String(form.get('gameFormat') || 'commander'); const playerCount = Number(form.get('playerCount')); const ownerCommanderCount = gameFormat === 'commander' ? Number(form.get('commanderCount')) : 1; const ownerCommanderNames = gameFormat === 'commander' ? commanderNamesFromForm(form, ownerCommanderCount) : ['']; const ownerCommanderColors = gameFormat === 'commander' ? commanderColorsFromFields(dom.createCommanderNames, ownerCommanderCount) : [[]]; const ownerPlayerId = 'P1'; const ownerName = String(form.get('name') || '').trim() || ownerPlayerId; const startingLife = Number(gameFormat === 'custom' ? form.get('customStartingLife') : form.get('startingLife')); if (!Number.isInteger(startingLife) || startingLife < 1 || startingLife > 999) { $('#customStartingLife').focus(); return; } const enteredRoundLimit = String(form.get('roundLimitMinutes') || '').trim(); const roundLimitMinutes = enteredRoundLimit ? Number(enteredRoundLimit) : null; const tableSkinId = String(form.get('tableSkinId') || ''); if (roundLimitMinutes !== null && (!Number.isInteger(roundLimitMinutes) || roundLimitMinutes < 1 || roundLimitMinutes > 999)) { dom.roundLimitMinutes.focus(); return; } const config = { playerCount, startingLife, gameFormat, ownerPlayerId, ownerName, ownerCommanderCount, ownerCommanderNames, ownerCommanderColors, roundLimitMinutes, tableSkinId }; if (dom.localSimulation.checked) return beginLocalGame({ ...config, localSimulation: true, podCode: 'LOCAL' }); try { const result = await transport.createRoom({ playerCount, startingLife: config.startingLife, gameFormat, commanderCount: ownerCommanderCount, commanderNames: ownerCommanderNames, commanderColors: ownerCommanderColors, name: ownerName, roundLimitMinutes, tableSkinId }); showSharedGame(result.snapshot); } catch (error) { showError(error); } });
...
$('#recoverPodButton').addEventListener('click', async () => { const code = latestRecoverableHostPodCode(); if (!code) return showError(new Error('This phone does not have a saved host pod to recover.')); try { const { snapshot } = await transport.restoreRoom(code); const host = snapshot.seats[snapshot.hostSeatId]; const result = await transport.claimRoom({ code, seatId: snapshot.hostSeatId, name: host.name, commanderCount: host.commanderCount, commanderNames: host.commanderNames, commanderColors: host.commanderColors }); showSharedGame(result.snapshot); } catch (error) { showError(error); } });
dom.showJoinQrButton.addEventListener('click', openJoinQr); dom.showJoinQrMenuButton.addEventListener('click', () => { dom.gameMenu.hidden = true; openJoinQr(); });
dom.shareJoinLinkButton.addEventListener('click', async () => { try { if (!navigator.share) throw new Error('Sharing is not available on this phone.'); await navigator.share({ title: 'Join my Fivefold Arc table', text: `Join pod ${state.podCode}`, url: joinLink() }); } catch (error) { if (error.name !== 'AbortError') showError(error); } });
dom.copyJoinLinkButton.addEventListener('click', () => copyJoinLink(dom.copyJoinLinkButton));
dom.copyLobbyJoinLinkButton.addEventListener('click', () => copyJoinLink(dom.copyLobbyJoinLinkButton));
function dialAngle(event) { const rect = dom.dialGesture.getBoundingClientRect(); return Math.atan2(event.clientY - (rect.top + rect.height / 2), event.clientX - (rect.left + rect.width / 2)) * 180 / Math.PI; }
...
function moveDialDrag(event) {
  ...
  const nextAngle = dialAngle(event); dialPointer.accumulated += normalizeDialAngle(nextAngle - dialPointer.angle); dialPointer.angle = nextAngle;
  // Twenty degrees is deliberate enough to reject normal finger jitter, while
...
dom.moreButton.addEventListener('click', () => { dom.gameMenu.hidden = !dom.gameMenu.hidden; dom.moreButton.setAttribute('aria-expanded', String(!dom.gameMenu.hidden)); }); dom.coinTossButton.addEventListener('click', () => { dom.gameMenu.hidden = true; dom.moreButton.setAttribute('aria-expanded', 'false'); tossCoin(); }); dom.declareWinnerButton.addEventListener('click', openDeclareWinner); dom.tossAgainButton.addEventListener('click', () => tossCoin()); dom.saveGameButton.addEventListener('click', saveGameToHistory); myGamesButton.addEventListener('click', openMyProfile); myGamesSignInButton.addEventListener('click', async () => { try { myGamesStatus.textContent = 'Signing in…'; await finishAccountSignIn(await googleAccountToken()); } catch (error) { myGamesStatus.textContent = error?.message || 'Sign-in did not work.'; } }); $('#resetButton').addEventListener('click', () => { dom.gameMenu.hidden = true; openResetDialog(); }); dom.confirmResetButton.addEventListener('click', resetGame);
...
myDecksSignInButton.addEventListener('click', () => showMyDecks(true)); saveAccountPreferencesButton.addEventListener('click', () => void saveAccountPreferences());
dom.tableSkinMenu.addEventListener('change', async () => { if (!state || (!state.localSimulation && transport.seatId !== state.hostSeatId)) return; const tableSkinId = dom.tableSkinMenu.value; try { if (state.localSimulation) { state.tableSkinId = tableSkinId; activeGameSkinId = ''; await applySelectedAppearance(); render(); } else await transport.setTableSkin(tableSkinId); } catch (error) { showError(error); render(); } });
createDeckButton.addEventListener('click', createDeck); signOutButton.addEventListener('click', () => void signOut());
myGamesContent.addEventListener('click', event => { const id = event.target.closest('[data-remove-game]')?.dataset.removeGame; if (id) void removeAccountItem('games', id); }); myDecksContent.addEventListener('click', event => { const editId = event.target.closest('[data-edit-deck]')?.dataset.editDeck; const removeId = event.target.closest('[data-remove-deck]')?.dataset.removeDeck; const favoriteId = event.target.closest('[data-toggle-favorite]')?.dataset.toggleFavorite; const archiveId = event.target.closest('[data-toggle-archive]')?.dataset.toggleArchive; const exportId = event.target.closest('[data-export-deck]')?.dataset.exportDeck; if (editId) beginDeckEdit(editId); else if (removeId) void removeAccountItem('decks', removeId); else if (favoriteId) { const label = event.target.textContent === 'Favorite'; void updateSavedDeck(favoriteId, { favorite: label }).catch(error => { myDecksStatus.textContent = error.message; }); } else if (archiveId) { const label = event.target.textContent === 'Archive'; void updateSavedDeck(archiveId, { archived: label }).catch(error => { myDecksStatus.textContent = error.message; }); } else if (exportId) void exportDeckSummary(exportId).catch(error => { myDecksStatus.textContent = error.message; }); });
exportAccountButton.addEventListener('click', () => { void exportAccountData().catch(error => { myGamesStatus.textContent = error.message; }); });
deleteAccountButton.addEventListener('click', () => { void deleteAccountData().catch(error => { myGamesStatus.textContent = error.message; }); });
dom.cardCameraButton.addEventListener('click', openCardCamera); dom.cardAdvisorForm.addEventListener('submit', event => { if (event.submitter?.value === 'close') return; event.preventDefault(); lookupCardTitles(); });
...
dom.markDevelopmentButton.addEventListener('click', async () => { try { await transport.setSessionKind(state.sessionKind === 'development' ? 'standard' : 'development'); } catch (error) { showError(error); } });
...
transport.addEventListener('protocolerror', () => recordClientDiagnostic('protocol_error'));
...
if (appearancePreviewMode) {
  ...
  const appearancePreviewStyle = document.createElement('style'); appearancePreviewStyle.textContent = 'html.appearance-preview[data-appearance-custom="true"] body{background:radial-gradient(circle at var(--appearance-position,50% 0%),color-mix(in srgb,var(--appearance-atmosphere,#000) calc(var(--appearance-strength,0) * 100%),transparent),transparent 58%),var(--bg);background-size:cover;background-position:center}html.appearance-preview .game-shell{pointer-events:none}html.appearance-preview[data-appearance-custom="true"] .counter-stage::after{border-style:var(--appearance-ring-style,solid)}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-ring="none"] .counter-stage::after,html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-ring="none"] .counter-sigil{display:none}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-ring="orbit"] .counter-stage::after{border-style:dashed}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-seat="flat"] .pod-seat{border-radius:0;box-shadow:none}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-seat="glass"] .pod-seat{background:color-mix(in srgb,var(--surface) 72%,transparent);backdrop-filter:blur(7px)}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-button="glass"] .end-turn-button{background:color-mix(in srgb,var(--brass) 36%,var(--surface));color:var(--ink);backdrop-filter:blur(8px)}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-button="outline"] .end-turn-button{background:transparent;color:var(--brass)}html.appearance-preview[data-appearance-custom="true"] .mode-nav img{width:22px;height:22px;object-fit:contain}html.appearance-preview[data-appearance-custom="true"] .counter-sigil img{width:100%;height:100%;object-fit:contain}'; document.head.append(appearancePreviewStyle);
  const appearanceSeatStyle = document.createElement('style'); appearanceSeatStyle.textContent = 'html.appearance-preview[data-appearance-custom="true"] #gameView .pod-seat{border-color:color-mix(in srgb,#5b594f calc(var(--appearance-seat-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--appearance-seat-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-seat-blur,0) * 1px))}html.appearance-preview[data-appearance-custom="true"] #gameView[data-appearance-seat="glass"] .pod-seat{background:color-mix(in srgb,var(--surface) calc(var(--appearance-seat-opacity,100) * .72%),transparent)}'; document.head.append(appearanceSeatStyle);
  const appearanceButtonStyle = document.createElement('style'); appearanceButtonStyle.textContent = 'html.appearance-preview[data-appearance-custom="true"] #gameView .end-turn-button{border-color:color-mix(in srgb,var(--brass) calc(var(--appearance-button-border,70) * 1%),transparent);background:color-mix(in srgb,var(--brass) calc(var(--appearance-button-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-button-blur,0) * 1px))}html.appearance-preview[data-appearance-custom="true"] #gameView .adjust-controls button{border-color:color-mix(in srgb,#62583f calc(var(--appearance-life-button-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--appearance-life-button-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-life-button-blur,0) * 1px))}html.appearance-preview[data-appearance-custom="true"] #gameView .custom-life-button{border-color:color-mix(in srgb,#62583f calc(var(--appearance-custom-life-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--appearance-custom-life-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-custom-life-blur,0) * 1px))}html.appearance-preview[data-appearance-custom="true"] #gameView .commander-tax-quick button{border-color:color-mix(in srgb,#62583f calc(var(--appearance-commander-tax-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--appearance-commander-tax-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-commander-tax-blur,0) * 1px))}'; document.head.append(appearanceButtonStyle);
  const appearanceCommanderDamageStyle = document.createElement('style'); appearanceCommanderDamageStyle.textContent = 'html.appearance-preview[data-appearance-custom="true"] #gameView .source-panel .selected-source-button,html.appearance-preview[data-appearance-custom="true"] #gameView .source-panel .inline-commander-choice{border-color:color-mix(in srgb,#62583f calc(var(--appearance-commander-damage-border,70) * 1%),transparent);background:color-mix(in srgb,var(--surface) calc(var(--appearance-commander-damage-opacity,100) * 1%),transparent);backdrop-filter:blur(calc(var(--appearance-commander-damage-blur,0) * 1px))}'; document.head.append(appearanceCommanderDamageStyle);
  const builtInSealMarkup = document.querySelector('.counter-sigil')?.innerHTML || ''; let activeAppearanceSkin = null;
  const appearanceUxStyle = document.createElement('style'); appearanceUxStyle.textContent = 'html.appearance-preview #gameView{font-family:var(--appearance-font,inherit)}html.appearance-preview #gameView .pod-seat{border-radius:var(--appearance-seat-radius,8px)}html.appearance-preview #gameView .pod-seat .seat-name{font-size:calc(.72rem * var(--appearance-seat-text-scale,1))}html.appearance-preview #gameView .pod-seat .seat-life{font-size:calc(1.82rem * var(--appearance-seat-text-scale,1))}html.appearance-preview #gameView .turn-banner p{font-size:calc(.67rem * var(--appearance-label-text-scale,1))}html.appearance-preview #gameView .turn-banner strong{font-size:calc(.92rem * var(--appearance-label-text-scale,1))}html.appearance-preview #gameView .turn-banner span{font-size:calc(.62rem * var(--appearance-label-text-scale,1))}html.appearance-preview #gameView .end-turn-button{font-size:calc(.82rem * var(--appearance-label-text-scale,1))}html.appearance-preview #gameView .mode-nav small{font-size:calc(clamp(.48rem,2.4vw,.65rem) * var(--appearance-label-text-scale,1))}html.appearance-preview #gameView .end-turn-button,html.appearance-preview #gameView .adjust-controls button{border-radius:var(--appearance-button-radius,6px)}html.appearance-preview #gameView .counter-readout{translate:0 var(--appearance-counter-offset,0);position:relative;z-index:3}html.appearance-preview #gameView .counter-sigil{translate:none;scale:1;transform:translate(-50%,-43%) translate(var(--appearance-seal-offset-x,0),var(--appearance-seal-offset-y,0)) scale(var(--appearance-seal-scale,1)) rotate(var(--appearance-seal-rotation,0deg));transform-origin:center}html.appearance-preview #gameView .main-value{translate:var(--appearance-life-number-offset-x,0) var(--appearance-life-number-offset-y,0);scale:var(--appearance-life-number-scale,1);transform-origin:center}html.appearance-preview #gameView #turnActions{translate:0 var(--appearance-turn-offset,0)}html.appearance-preview #gameView .adjust-controls{translate:0 var(--appearance-controls-offset,0)}html.appearance-preview .appearance-overlay{position:absolute;inset:0;z-index:2;width:100%;height:100%;object-fit:cover;pointer-events:none;mix-blend-mode:var(--appearance-overlay-blend,normal);opacity:var(--appearance-overlay-opacity,.3);transform:scale(var(--appearance-overlay-scale,1));transform-origin:center}html.appearance-preview .appearance-dial-art{border-radius:50%;transform:scale(var(--appearance-overlay-scale,1)) rotate(var(--appearance-dial-art-rotation,0deg))}html.appearance-preview .pod-seat{position:relative;overflow:hidden}html.appearance-preview .pod-seat>.seat-name,html.appearance-preview .pod-seat>.seat-life,html.appearance-preview .dial-controls>*:not(.appearance-overlay){position:relative;z-index:3}html.appearance-preview .pod-seat>.identity-rail{z-index:3}html.appearance-preview .counter-stage>.dial-controls{z-index:3}'; document.head.append(appearanceUxStyle);
  function applyAppearanceOverlays(skin) { document.querySelectorAll('.appearance-overlay').forEach(node => node.remove()); const add = (target, data, slot) => { if (!target || !data) return; const image = document.createElement('img'); image.className = `appearance-overlay${slot === 'dial' ? ' appearance-dial-art' : ''}`; image.alt = ''; image.src = data; image.style.setProperty('--appearance-overlay-opacity', String(Math.min(70, Math.max(0, Number(skin.overlayOpacity?.[slot]) || 0)) / 100)); image.style.setProperty('--appearance-overlay-scale', String(Math.min(160, Math.max(50, Number(skin.overlayScale?.[slot]) || 100)) / 100)); image.style.setProperty('--appearance-overlay-blend', skin.overlayBlend?.[slot] || 'normal'); if (slot === 'dial') image.style.setProperty('--appearance-dial-art-rotation', skin.dialArtSpin ? 'var(--dial-seal-rotation,0deg)' : '0deg'); target.append(image); }; document.querySelectorAll('.pod-seat').forEach(seat => add(seat, skin.overlayData?.seat, 'seat')); add(document.querySelector('.counter-stage'), skin.overlayData?.counter, 'counter'); add(document.querySelector('.dial-gesture'), skin.overlayData?.dial, 'dial'); }
  // Layout transforms apply only to named outer regions. They never change
  // child positioning, handlers, or document flow inside the game renderer.
  const appearanceComponentLayoutStyle = document.createElement('style'); appearanceComponentLayoutStyle.textContent = 'html.appearance-preview #gameView .pod-strip{translate:var(--appearance-pod-strip-x,0) var(--appearance-pod-strip-y,0);scale:var(--appearance-pod-strip-scale,1)}html.appearance-preview #gameView .counter-readout{translate:var(--appearance-counter-readout-x,0) calc(var(--appearance-counter-offset,0px) + var(--appearance-counter-readout-y,0px));scale:var(--appearance-counter-readout-scale,1)}html.appearance-preview #gameView .dial-gesture{translate:var(--appearance-dial-surface-x,0) var(--appearance-dial-surface-y,0);scale:var(--appearance-dial-surface-scale,1)}html.appearance-preview #gameView .source-panel{translate:var(--appearance-commander-damage-x,0) var(--appearance-commander-damage-y,0);scale:var(--appearance-commander-damage-scale,1)}html.appearance-preview #gameView #turnActions{translate:var(--appearance-turn-actions-x,0) calc(var(--appearance-turn-offset,0px) + var(--appearance-turn-actions-y,0px));scale:var(--appearance-turn-actions-scale,1)}html.appearance-preview #gameView .adjust-controls{translate:var(--appearance-adjust-controls-x,0) calc(var(--appearance-controls-offset,0px) + var(--appearance-adjust-controls-y,0px));scale:var(--appearance-adjust-controls-scale,1)}html.appearance-preview #gameView .custom-life-button{translate:var(--appearance-custom-life-x,0) var(--appearance-custom-life-y,0);scale:var(--appearance-custom-life-scale,1)}html.appearance-preview #gameView .commander-tax-quick{translate:var(--appearance-commander-tax-x,0) var(--appearance-commander-tax-y,0);scale:var(--appearance-commander-tax-scale,1)}html.appearance-preview #gameView .mode-nav{translate:var(--appearance-mode-nav-x,0) var(--appearance-mode-nav-y,0);scale:var(--appearance-mode-nav-scale,1)}'; document.head.append(appearanceComponentLayoutStyle);
  // A desktop iframe has no iPhone safe-area environment values. Device
  ...
  window.addEventListener('message', event => {
    ...
    const root = document.documentElement, game = dom.game, customAppearance = !skin.useGameBase; root.dataset.appearanceCustom = String(customAppearance); game.dataset.appearanceCustom = String(customAppearance); appearanceUxStyle.disabled = !customAppearance; root.style.colorScheme = ['light','dark'].includes(skin.previewColorScheme) ? skin.previewColorScheme : ''; ['button','lifeButton','customLife','commanderTax','commanderDamage'].forEach(prefix => { const css = prefix.replace(/[A-Z]/g, letter => `-${letter.toLowerCase()}`), opacity = Math.min(100, Math.max(35, Number(skin[`${prefix}Opacity`]) || 100)), blur = Math.min(16, Math.max(0, Number(skin[`${prefix}Blur`]) || 0)), border = Number.isFinite(Number(skin[`${prefix}Border`])) ? Math.min(100, Math.max(0, Number(skin[`${prefix}Border`]))) : 70; root.style.setProperty(`--appearance-${css}-opacity`, String(opacity)); root.style.setProperty(`--appearance-${css}-blur`, String(blur)); root.style.setProperty(`--appearance-${css}-border`, String(border)); });
    ...
    root.style.setProperty('--appearance-preview-safe-bottom', `${Math.min(48, Math.max(0, Number(skin.previewSafeBottom) || 0))}px`);
    ...
    interfaceStyle = skin.interfaceStyle === 'dial' ? 'dial' : 'button'; root.style.setProperty('--appearance-font', skin.fontFamily === 'serif' ? 'Georgia,serif' : skin.fontFamily === 'rounded' ? 'ui-rounded,system-ui,sans-serif' : skin.fontFamily === 'mono' ? 'ui-monospace,monospace' : 'inherit'); root.style.setProperty('--appearance-label-text-scale', String(Math.min(115, Math.max(85, Number(skin.labelTextScale) || 100)) / 100)); root.style.setProperty('--appearance-seat-text-scale', String(Math.min(115, Math.max(85, Number(skin.seatTextScale) || 100)) / 100)); root.style.setProperty('--appearance-seat-radius', `${Math.min(24, Math.max(0, Number(skin.seatRadius) || 8))}px`); root.style.setProperty('--appearance-button-radius', `${Math.min(24, Math.max(0, Number(skin.buttonRadius) || 8))}px`); root.style.setProperty('--appearance-counter-offset', `${Math.min(18, Math.max(-18, Number(skin.counterOffsetY) || 0))}px`); root.style.setProperty('--appearance-seal-scale', String(Math.min(150, Math.max(60, Number(skin.centerSealScale) || 100)) / 100)); root.style.setProperty('--appearance-seal-offset-x', `${Math.min(18, Math.max(-18, Number(skin.centerSealOffsetX) || 0))}px`); root.style.setProperty('--appearance-seal-offset-y', `${Math.min(18, Math.max(-18, Number(skin.centerSealOffsetY) || 0))}px`); root.style.setProperty('--appearance-seal-rotation', interfaceStyle === 'dial' ? 'var(--dial-seal-rotation,0deg)' : '0deg'); root.style.setProperty('--appearance-life-number-scale', String(Math.min(140, Math.max(75, Number(skin.lifeNumberScale) || 100)) / 100)); root.style.setProperty('--appearance-life-number-offset-x', `${Math.min(14, Math.max(-14, Number(skin.lifeNumberOffsetX) || 0))}px`); root.style.setProperty('--appearance-life-number-offset-y', `${Math.min(14, Math.max(-14, Number(skin.lifeNumberOffsetY) || 0))}px`); root.style.setProperty('--appearance-turn-offset', `${Math.min(12, Math.max(-12, Number(skin.turnOffsetY) || 0))}px`); root.style.setProperty('--appearance-controls-offset', `${Math.min(12, Math.max(-12, Number(skin.controlsOffsetY) || 0))}px`);
    const componentLayout = skin.componentLayout || {};
    Object.entries({ podStrip: 'pod-strip', counterReadout: 'counter-readout', dialSurface: 'dial-surface', commanderDamage: 'commander-damage', turnActions: 'turn-actions', adjustControls: 'adjust-controls', customLife: 'custom-life', commanderTax: 'commander-tax', modeNav: 'mode-nav' }).forEach(([key, css]) => {
      const placement = componentLayout[key] || {};
      root.style.setProperty(`--appearance-${css}-x`, `${Math.min(100, Math.max(-100, Number(placement.x) || 0))}px`);
      root.style.setProperty(`--appearance-${css}-y`, `${Math.min(140, Math.max(-140, Number(placement.y) || 0))}px`);
      root.style.setProperty(`--appearance-${css}-scale`, String(Math.min(140, Math.max(70, Number(placement.scale) || 100)) / 100));
    });
    ...
    document.body.style.backgroundSize = skin.backgroundData ? `${Math.min(200, Math.max(50, Number(skin.backgroundScale) || 100))}% auto` : '';
    document.body.style.backgroundPosition = skin.backgroundData ? `${Math.min(100, Math.max(0, Number(skin.backgroundPositionX) || 50))}% ${Math.min(100, Math.max(0, Number(skin.backgroundPositionY) || 50))}%` : '';
    ...
    renderConnection(skin.previewState === 'offline' ? 'disconnected' : 'local'); render(); applyAppearanceOverlays(skin);
  });
  if (phoneLayoutSessionId) {
    const editorStyle = document.createElement('style'); editorStyle.textContent = 'html.appearance-preview .phone-layout-editor{position:fixed;z-index:30;right:10px;bottom:calc(12px + env(safe-area-inset-bottom));left:10px;max-height:48dvh;overflow:auto;padding:12px;border:1px solid var(--brass);border-radius:10px;background:rgb(16 16 15 / .96);box-shadow:0 14px 40px #000c;pointer-events:auto;color:var(--ink)}html.appearance-preview .phone-layout-editor header{display:flex;justify-content:space-between;gap:8px;align-items:center}html.appearance-preview .phone-layout-editor strong{color:var(--brass);font-size:.82rem}html.appearance-preview .phone-layout-editor button,.phone-layout-editor-launcher{min-height:32px;padding:5px 8px;border:1px solid var(--brass-dark);border-radius:5px;background:var(--surface);color:var(--ink);font-weight:800}.phone-layout-editor-launcher{position:fixed;z-index:30;right:10px;bottom:calc(12px + env(safe-area-inset-bottom));pointer-events:auto}.phone-layout-editor details{margin-top:8px;border-top:1px solid var(--line)}.phone-layout-editor summary{padding:8px 0;color:var(--brass);font-size:.75rem;font-weight:800}.phone-layout-editor label{display:grid;grid-template-columns:1fr auto;gap:6px;align-items:center;margin:8px 0;color:var(--ink);font-size:.72rem}.phone-layout-editor input,.phone-layout-editor select{accent-color:var(--brass)}.phone-layout-editor input[type=range]{width:120px}.phone-layout-editor output{color:var(--brass);font-size:.7rem}'; document.head.append(editorStyle);
    const editor = document.createElement('aside'); editor.className = 'phone-layout-editor'; editor.setAttribute('aria-label', 'Phone Layout Editor'); editor.innerHTML = '<header><strong>Phone Layout Editor</strong><button type="button" data-phone-editor-close>Hide</button></header><details open><summary>Choose layout</summary><label>Controls <select data-phone-layout-style><option value="button">Button controls</option><option value="dial">Dial controls</option></select></label></details><details open><summary>Center readout</summary><label>Life total size <output data-phone-output="lifeNumberScale"></output><input data-phone-layout-field="lifeNumberScale" type="range" min="75" max="140" step="5"></label><label>Life total left/right <output data-phone-output="lifeNumberOffsetX"></output><input data-phone-layout-field="lifeNumberOffsetX" type="range" min="-14" max="14" step="2"></label><label>Life total up/down <output data-phone-output="lifeNumberOffsetY"></output><input data-phone-layout-field="lifeNumberOffsetY" type="range" min="-14" max="14" step="2"></label><label>Seal size <output data-phone-output="centerSealScale"></output><input data-phone-layout-field="centerSealScale" type="range" min="60" max="150" step="5"></label></details><details><summary>Cards & controls</summary><label>Card corners <output data-phone-output="seatRadius"></output><input data-phone-layout-field="seatRadius" type="range" min="0" max="24" step="2"></label><label>Button corners <output data-phone-output="buttonRadius"></output><input data-phone-layout-field="buttonRadius" type="range" min="0" max="24" step="2"></label><label>Turn position <output data-phone-output="turnOffsetY"></output><input data-phone-layout-field="turnOffsetY" type="range" min="-12" max="12" step="2"></label><label>Life buttons <output data-phone-output="controlsOffsetY"></output><input data-phone-layout-field="controlsOffsetY" type="range" min="-12" max="12" step="2"></label></details>';
    const editorLauncher = document.createElement('button'); editorLauncher.type = 'button'; editorLauncher.className = 'phone-layout-editor-launcher'; editorLauncher.textContent = 'Edit layout'; editorLauncher.hidden = true; document.body.append(editor, editorLauncher);
    const phonePlacementDetails = document.createElement('details'); phonePlacementDetails.innerHTML = '<summary>Place game regions</summary><p class="phone-layout-note">Choose a real game region, then adjust its visual position. Game actions stay protected.</p><label>Region <select data-phone-placement-region><option value="podStrip">Player cards</option><option value="counterReadout">Life total and center</option><option value="dialSurface">Dial surface</option><option value="commanderDamage">Commander damage cards</option><option value="turnActions">End turn area</option><option value="adjustControls">Life change buttons</option><option value="customLife">Custom life button</option><option value="commanderTax">Commander card</option><option value="modeNav">Bottom navigation</option></select></label><label>Left / right <output data-phone-placement-output="x"></output><input data-phone-placement-field="x" type="range" min="-100" max="100" step="2"></label><label>Up / down <output data-phone-placement-output="y"></output><input data-phone-placement-field="y" type="range" min="-140" max="140" step="2"></label><label>Size <output data-phone-placement-output="scale"></output><input data-phone-placement-field="scale" type="range" min="70" max="140" step="2"></label><button type="button" data-phone-placement-reset>Reset selected region</button>'; editor.append(phonePlacementDetails);
    let phoneSaveTimer = null;
    const phoneLayout = () => activeAppearanceSkin?.modeLayouts?.[activeAppearanceSkin.interfaceStyle === 'dial' ? 'dial' : 'button'];
    const refreshPhoneEditor = () => { const layout = phoneLayout(); if (!layout) return; editor.querySelector('[data-phone-layout-style]').value = activeAppearanceSkin.interfaceStyle === 'dial' ? 'dial' : 'button'; editor.querySelectorAll('[data-phone-layout-field]').forEach(input => { input.value = layout[input.dataset.phoneLayoutField] ?? input.min; const output = editor.querySelector(`[data-phone-output="${input.dataset.phoneLayoutField}"]`); if (output) output.value = `${input.value}${input.dataset.phoneLayoutField.endsWith('Scale') ? '%' : 'px'}`; }); const region = editor.querySelector('[data-phone-placement-region]')?.value || 'podStrip', placement = layout.componentLayout?.[region] || { x: 0, y: 0, scale: 100 }; editor.querySelectorAll('[data-phone-placement-field]').forEach(input => { input.value = placement[input.dataset.phonePlacementField] ?? 0; const output = editor.querySelector(`[data-phone-placement-output="${input.dataset.phonePlacementField}"]`); if (output) output.value = `${input.value}${input.dataset.phonePlacementField === 'scale' ? '%' : 'px'}`; }); };
    const savePhoneLayout = () => { clearTimeout(phoneSaveTimer); phoneSaveTimer = setTimeout(async () => { try { await fetch(`/api/appearance-studio/phone-layout/${phoneLayoutSessionId}`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ skin: activeAppearanceSkin }) }); } catch {} }, 180); };
    const applyPhoneLayout = () => { window.postMessage({ type: 'fivefold-arc:appearance-skin', skin: activeAppearanceSkin }, location.origin); refreshPhoneEditor(); savePhoneLayout(); };
    editor.querySelector('[data-phone-layout-style]').addEventListener('input', event => { activeAppearanceSkin.interfaceStyle = event.target.value === 'dial' ? 'dial' : 'button'; applyPhoneLayout(); }); editor.querySelectorAll('[data-phone-layout-field]').forEach(input => input.addEventListener('input', event => { const layout = phoneLayout(); if (!layout) return; layout[event.target.dataset.phoneLayoutField] = Number(event.target.value); activeAppearanceSkin[event.target.dataset.phoneLayoutField] = Number(event.target.value); applyPhoneLayout(); })); editor.querySelector('[data-phone-placement-region]').addEventListener('input', refreshPhoneEditor); editor.querySelectorAll('[data-phone-placement-field]').forEach(input => input.addEventListener('input', event => { const layout = phoneLayout(); if (!layout) return; const region = editor.querySelector('[data-phone-placement-region]').value; layout.componentLayout ||= {}; layout.componentLayout[region] ||= { x: 0, y: 0, scale: 100 }; layout.componentLayout[region][event.target.dataset.phonePlacementField] = Number(event.target.value); applyPhoneLayout(); })); editor.querySelector('[data-phone-placement-reset]').addEventListener('click', () => { const layout = phoneLayout(); if (!layout) return; layout.componentLayout ||= {}; layout.componentLayout[editor.querySelector('[data-phone-placement-region]').value] = { x: 0, y: 0, scale: 100 }; applyPhoneLayout(); }); editor.querySelector('[data-phone-editor-close]').addEventListener('click', () => { editor.hidden = true; editorLauncher.hidden = false; }); editorLauncher.addEventListener('click', () => { editor.hidden = false; editorLauncher.hidden = true; });
    let phoneLayoutUpdatedAt = 0;
    const loadPhoneLayout = async () => { try { const response = await fetch(`/api/appearance-studio/phone-layout/${phoneLayoutSessionId}`); if (!response.ok) return; const session = await response.json(); if (session.updatedAt <= phoneLayoutUpdatedAt) return; phoneLayoutUpdatedAt = session.updatedAt; window.postMessage({ type: 'fivefold-arc:appearance-skin', skin: session.skin }, location.origin); setTimeout(refreshPhoneEditor, 0); } catch {} }; void loadPhoneLayout(); setInterval(loadPhoneLayout, 1200);
  }
