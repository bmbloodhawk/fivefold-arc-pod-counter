...
async function showMyGames(signIn = false) {
  ...
  try {
    ...
    const history = (await response.json()).history; enterApp(true); void loadSetupDecks(); exportAccountButton.hidden = false; deleteAccountButton.hidden = false; myGamesStatus.textContent = `${history.wins} wins in ${history.gamesPlayed} games${history.gamesPlayed ? ` · ${Math.round(history.winRate * 100)}% win rate` : ''}`;
...
async function createDeck() {
  ...
  try { const response = await fetch('/api/account/decks', { method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ commanderNames, name: deckName.value }) }); if (!response.ok) throw new Error('This deck could not be saved yet.'); deckCommanderName.value = ''; deckCommanderNameTwo.value = ''; deckName.value = ''; await showMyDecks(); } catch (error) { myDecksStatus.textContent = error?.message || 'This deck could not be saved yet.'; createDeckButton.disabled = false; }
}
...
myDecksSignInButton.addEventListener('click', () => showMyDecks(true));
