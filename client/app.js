...
async function finishAccountSignIn(token) {
  ...
  enterApp(true); void loadSetupDecks(); myGamesDialog.close(); showView(dom.landing);
}
...
function closeGameOverlays() { [dom.resetDialog, dom.connectionDialog, dom.coinTossDialog, dom.startingRollDialog, dom.turnCueDialog, dom.turnSoundDialog, dom.customLifeDialog, dom.commanderSourceDialog, dom.commanderCountDialog, dom.commanderTaxDialog, dom.victoryDialog, dom.declareWinnerDialog, dom.playtestNotesDialog, dom.playtestRecapDialog, dom.savedPlaytestsDialog, dom.cardCameraDialog].forEach(dialog => { if (dialog?.open) dialog.close(); }); }
...
$('#joinSignInDeckButton').addEventListener('click', openSignIn);
...
myGamesDialog.querySelector('form').addEventListener('submit', event => { if (event.submitter?.value !== 'close') event.preventDefault(); });
...
dom.victoryDialog.addEventListener('click', () => { if (victoryDismissReady && dom.victoryDialog.open) dom.victoryDialog.close('tap'); }); dom.victoryDialog.addEventListener('close', () => { if (dom.victoryDialog.returnValue === 'tap') openQuickFeedback(); });
