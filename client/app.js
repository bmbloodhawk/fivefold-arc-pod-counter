...
async function showMyGames(signIn = false) {
  ...
  try {
    ...
    const preferencesResponse = await fetch('/api/account/preferences', { headers: { authorization: `Bearer ${token}` } }); const preferences = preferencesResponse.ok ? (await preferencesResponse.json()).preferences : { preferredName: null }; accountPreferredName.value = preferences.preferredName || ''; if (preferences.preferredName) { dom.createName.value ||= preferences.preferredName; dom.joinName.value ||= preferences.preferredName; }
...
async function saveAccountPreferences() { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/preferences', { method: 'PUT', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ preferredName: accountPreferredName.value }) }); if (!response.ok) { myGamesStatus.textContent = 'Your preferences could not be saved yet.'; return; } dom.createName.value ||= accountPreferredName.value.trim(); dom.joinName.value ||= accountPreferredName.value.trim(); myGamesStatus.textContent = 'Preferences saved.'; }
...
myGamesContent.addEventListener('click', event => { const id = event.target.closest('[data-remove-game]')?.dataset.removeGame; if (id) void removeAccountItem('games', id); }); myDecksContent.addEventListener('click', event => { const id = event.target.closest('[data-remove-deck]')?.dataset.removeDeck; if (id) void removeAccountItem('decks', id); });
