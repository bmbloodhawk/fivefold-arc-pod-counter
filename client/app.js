...
function playTurnCue() {
  ...
  if (mode === 'sound' || mode === 'both') try {
    prepareTurnCueAudio(); if (!turnCueAudio || turnCueAudio.state === 'suspended') return; const oscillator = turnCueAudio.createOscillator(); const gain = turnCueAudio.createGain();
...
function closeGameOverlays() { [dom.resetDialog, dom.connectionDialog, dom.coinTossDialog, dom.startingRollDialog, dom.turnCueDialog, dom.customLifeDialog, dom.commanderSourceDialog, dom.commanderCountDialog, dom.commanderTaxDialog, dom.victoryDialog, dom.declareWinnerDialog, dom.playtestNotesDialog, dom.playtestRecapDialog, dom.savedPlaytestsDialog, dom.cardCameraDialog].forEach(dialog => { if (dialog?.open) dialog.close(); }); }
