...
async function showMyGames(signIn = false) {
...
function enterApp(signedIn = false) { accountChoice.hidden = true; playActions.hidden = false; myGamesButton.setAttribute('aria-disabled', String(!signedIn)); myDecksButton.setAttribute('aria-disabled', String(!signedIn)); renderSavedTables(); }
...
createAccountButton.addEventListener('click', async () => { try { await emailAccountToken(accountEmail.value, accountPassword.value, true); showMyGames(); } catch (error) { myGamesStatus.textContent = error?.message || 'Account creation did not work.'; } });
