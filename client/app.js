...
let selectedDeckId = sessionStorage.getItem('fivefold-arc:selected-deck') || ''; let setupDecks = []; let joinGameFormat = 'commander';
...
async function createDeck() {
  ...
  try { const colors = $$('input[name="deckColors"]:checked').map(input => input.value); const response = await fetch('/api/account/decks', { method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ commanderNames, name: deckName.value, colors, notes: deckNotes.value, favorite: deckFavorite.checked }) }); if (!response.ok) throw new Error('This deck could not be saved yet.'); deckCommanderName.value = ''; deckCommanderNameTwo.value = ''; deckName.value = ''; deckNotes.value = ''; deckFavorite.checked = false; $$('input[name="deckColors"]').forEach(input => { input.checked = false; }); await showMyDecks(); } catch (error) { myDecksStatus.textContent = error?.message || 'This deck could not be saved yet.'; createDeckButton.disabled = false; }
}
...
myGamesContent.addEventListener('click', event => { const id = event.target.closest('[data-remove-game]')?.dataset.removeGame; if (id) void removeAccountItem('games', id); }); myDecksContent.addEventListener('click', event => { const removeId = event.target.closest('[data-remove-deck]')?.dataset.removeDeck; const favoriteId = event.target.closest('[data-toggle-favorite]')?.dataset.toggleFavorite; const archiveId = event.target.closest('[data-toggle-archive]')?.dataset.toggleArchive; const exportId = event.target.closest('[data-export-deck]')?.dataset.exportDeck; if (removeId) void removeAccountItem('decks', removeId); else if (favoriteId) { const label = event.target.textContent === 'Favorite'; void updateSavedDeck(favoriteId, { favorite: label }).catch(error => { myDecksStatus.textContent = error.message; }); } else if (archiveId) { const label = event.target.textContent === 'Archive'; void updateSavedDeck(archiveId, { archived: label }).catch(error => { myDecksStatus.textContent = error.message; }); } else if (exportId) void exportDeckSummary(exportId).catch(error => { myDecksStatus.textContent = error.message; }); });
