...
function updateFormatSetup() { const commander = selectedGameFormat() === 'commander'; dom.createCommanderNames.previousElementSibling.hidden = !commander; dom.createCommanderNames.hidden = !commander; dom.createDeckField.hidden = !commander || !setupDecks.length; $('#createStepTwoNext').textContent = commander ? 'Next: commander' : 'Next: table settings'; $('#gameFormatHelp').textContent = commander ? '40 life with commander damage and commander tax.' : selectedGameFormat() === 'casual' ? '20 life by default. Commander damage and commander tax are off.' : 'Choose the life total and counters your table wants. Commander-only rules are off.'; const joiningCommander = joinGameFormat === 'commander'; dom.joinCommanderNames.previousElementSibling.hidden = !joiningCommander; dom.joinCommanderNames.hidden = !joiningCommander; dom.joinDeckField.hidden = !joiningCommander || !setupDecks.length; }
