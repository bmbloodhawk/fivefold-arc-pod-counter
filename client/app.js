...
async function finishAccountSignIn(token) {
  ...
  accountPreferredName.value = preferences.preferredName || ''; accountDefaultPlayerCount.value = String(preferences.defaultPlayerCount || 4); accountDefaultRoundLimit.value = preferences.defaultRoundLimitMinutes || ''; if (preferences.preferredName) { dom.createName.value ||= preferences.preferredName; dom.joinName.value ||= preferences.preferredName; } const playerDefault = $(`input[name="playerCount"][value="${preferences.defaultPlayerCount || 4}"]`); if (playerDefault) playerDefault.checked = true; dom.roundLimitMinutes.value ||= preferences.defaultRoundLimitMinutes || '';
...
async function showMyGames() {
  ...
  try {
    ...
    const preferencesResponse = await fetch('/api/account/preferences', { headers: { authorization: `Bearer ${token}` } }); const preferences = preferencesResponse.ok ? (await preferencesResponse.json()).preferences : { preferredName: null, defaultPlayerCount: 4, defaultRoundLimitMinutes: null }; accountPreferredName.value = preferences.preferredName || ''; accountDefaultPlayerCount.value = String(preferences.defaultPlayerCount || 4); accountDefaultRoundLimit.value = preferences.defaultRoundLimitMinutes || ''; accountDefaultPlayerCountField.hidden = false; accountDefaultRoundLimitField.hidden = false; if (preferences.preferredName) { dom.createName.value ||= preferences.preferredName; dom.joinName.value ||= preferences.preferredName; } const playerDefault = $(`input[name="playerCount"][value="${preferences.defaultPlayerCount || 4}"]`); if (playerDefault) playerDefault.checked = true; dom.roundLimitMinutes.value ||= preferences.defaultRoundLimitMinutes || '';
    ...
    const profileSettings = [accountDefaultPlayerCountField, accountDefaultRoundLimitField, saveAccountPreferencesButton, exportAccountButton, deleteAccountButton, signOutButton];
