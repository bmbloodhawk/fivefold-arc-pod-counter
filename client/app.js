...
async function showMyGames(signIn = false, returnToPodChoice = false) {
  myGamesContent.hidden = true; myGamesStatus.textContent = signIn ? 'Signing in…' : 'Loading your saved games…'; myGamesSignInButton.hidden = true;
  try {
    ...
    if (returnToPodChoice) { myGamesDialog.close(); showView(dom.landing); return; }
...
createAccountButton.addEventListener('click', async () => { try { await emailAccountToken(accountEmail.value, accountPassword.value, true); showMyGames(false, true); } catch (error) { myGamesStatus.textContent = error?.message || 'Account creation did not work.'; } });
