...
async function showMyGames(signIn = false, returnToPodChoice = false) {
  ...
  try {
    ...
    const profileLabels = ['Overview', 'Achievements', 'Deck stats', 'Games', 'Settings'];
    const profileSettings = [accountPreferredNameField, accountDefaultPlayerCountField, accountDefaultRoundLimitField, saveAccountPreferencesButton, exportAccountButton, deleteAccountButton];
    profileSettings.forEach(item => { item.hidden = true; });
    profileSections.forEach((section, index) => { section.dataset.profilePanel = String(index); section.hidden = index !== 0; });
    myGamesContent.insertAdjacentHTML('afterbegin', `<nav class="profile-tabs" aria-label="Profile sections">${profileLabels.filter((_, index) => profileSections[index]).map((label, index) => `<button type="button" data-profile-tab="${index}"${index ? '' : ' aria-current="page"'}>${label}</button>`).join('')}</nav>`);
    profileSections[1]?.querySelector('h3')?.replaceChildren('Achievements');
    myGamesContent.querySelectorAll('[data-profile-tab]').forEach(button => button.addEventListener('click', () => { const active = Number(button.dataset.profileTab); profileSections.forEach((section, index) => { section.hidden = index !== active; }); profileSettings.forEach(item => { item.hidden = active !== profileSections.length; }); myGamesContent.querySelectorAll('[data-profile-tab]').forEach(tab => tab.toggleAttribute('aria-current', Number(tab.dataset.profileTab) === active)); }));
