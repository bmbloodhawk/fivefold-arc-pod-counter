...
function renderCommanderNameFields(container, count, names = [], identities = []) {
  if (!container) return;
  const current = [...container.querySelectorAll('input[name^="commanderName"]')].map(input => input.value);
  container.innerHTML = Array.from({ length: count }, (_, slot) => `<div class="commander-identity-field"><label class="select-field">Commander ${count === 2 ? slot === 0 ? 'A' : 'B' : ''} name <small>(optional)</small><input name="commanderName${slot}" type="text" maxlength="60" autocomplete="off" placeholder="e.g. Atraxa, Praetors’ Voice" value="${escapeHtml(current[slot] ?? names[slot] ?? '')}"></label><small class="field-help">Color identity is looked up automatically from the commander name.</small></div>`).join('');
}
...
dom.commanderCountForm.addEventListener('submit', async event => { if (event.submitter?.value === 'confirm') { const form = new FormData(dom.commanderCountForm); const count = Number(form.get('gameCommanderCount')); const names = commanderNamesFromForm(form, count); updateCommanderSetup(count, names, await commanderColorsFromNames(names)); } }); dom.declareWinnerForm.addEventListener('submit', event => { if (event.submitter?.value === 'confirm') declareWinner(); }); dom.connectionButton.addEventListener('click', () => dom.connectionDialog.showModal());
