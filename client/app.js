...
function renderTurnFlow() {
  ...
  if (!isStarted) {
    ...
    const shareCode = state.podCode && state.podCode !== 'LOCAL' ? `Share code ${state.podCode}. ` : '';
...
dom.copyJoinLinkButton.addEventListener('click', async () => { try { await navigator.clipboard.writeText(joinLink()); dom.copyJoinLinkButton.textContent = 'Copied'; setTimeout(() => { dom.copyJoinLinkButton.textContent = 'Copy link'; }, 1800); } catch { showError(new Error('Copy is not available here. Scan the QR code or use the pod code shown above.')); } });
