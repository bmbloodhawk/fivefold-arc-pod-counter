...
let selectedDeckId = sessionStorage.getItem('fivefold-arc:selected-deck') || ''; let setupDecks = []; let joinGameFormat = 'commander'; let savedDecks = []; let editingDeckId = null;
...
async function loadSetupDecks() {
  try { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/decks', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return; const { decks } = await response.json(); setupDecks = decks; const options = `<option value="">No deck selected</option>${decks.map(deck => `<option value="${escapeHtml(deck.deckId)}">${escapeHtml(`${deck.favorite ? '★ ' : ''}${[deck.commanderName, deck.name, deck.colors?.join('')].filter(Boolean).join(' · ')}`)}</option>`).join('')}`; [dom.createDeck, dom.joinDeck].forEach(select => { select.innerHTML = options; select.value = selectedDeckId; }); $('#joinSignInDeckButton').hidden = true; updateFormatSetup(); } catch {} }
...
async function showMyGames(signIn = false, continueToCreate = false) {
  ...
  try {
    ...
    if (continueToCreate) { myGamesDialog.close(); showCreateStep(1); showView(dom.create); return; }
