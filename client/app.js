import { RealtimeAdapter, apiBaseFromPage } from './realtime.js?v=24';
...
function playerTemplate(number, startingLife, commanderCount, sources, commanderNames = []) {
  return { id: `P${number}`, name: `P${number}`, commanderCount, commanderNames: Array.from({ length: commanderCount }, (_, slot) => commanderNames[slot] || ''), life: startingLife, poison: 0, commanderDamage: blankDamage(sources), energy: 0, storm: 0, generic: 0, connectionStatus: 'connected', eliminated: false, lethalCause: null, warning: null };
}
function createState({ playerCount = 4, startingLife = 40, ownerPlayerId = 'P1', ownerName = ownerPlayerId, ownerCommanderCount = 1, ownerCommanderNames = [], roundLimitMinutes = null, localSimulation = true, podCode = 'LOCAL' } = {}) {
  const counts = Array.from({ length: playerCount }, (_, index) => `P${index + 1}` === ownerPlayerId ? ownerCommanderCount : 1);
  const players = counts.map((count, index) => playerTemplate(index + 1, startingLife, count, [], `P${index + 1}` === ownerPlayerId ? ownerCommanderNames : []));
...
function normaliseSnapshotSources(snapshot) {
  ...
  if (raw?.length) return raw.map((source, index) => {
    ...
    return { id: String(source.id ?? source.commanderId ?? `source-${index}`), label, ownerLabel: source.ownerLabel || '', commanderName: source.commanderName || '', ownerPlayerId, slot: source.slot || (label.match(/\s([AB])$/)?.[1] || 'A') };
  });
  return sourcesFromPlayers(snapshot.seats.map(seat => ({ id: `P${seat.seatId + 1}`, name: seat.name, commanderCount: seat.commanderCount === 2 ? 2 : 1, commanderNames: seat.commanderNames || [] })));
}
...
function renderCommanderNameFields(container, count, names = []) {
  if (!container) return;
  const current = [...container.querySelectorAll('input')].map(input => input.value);
  container.innerHTML = Array.from({ length: count }, (_, slot) => `<label class="select-field">Commander ${count === 2 ? slot === 0 ? 'A' : 'B' : ''} name <small>(optional)</small><input name="commanderName${slot}" type="text" maxlength="60" autocomplete="off" placeholder="e.g. Atraxa, Praetors’ Voice" value="${escapeHtml(current[slot] ?? names[slot] ?? '')}"></label>`).join('');
}
...
async function updateCommanderSetup(count, names) {
  const player = state.localSimulation ? activePlayer() : state.players.find(item => item.id === state.ownerPlayerId);
  if (transport.status === 'local') { const previousSources = state.commanderSources; player.commanderCount = count; player.commanderNames = names; state.commanderSources = sourcesFromPlayers(state.players); remapLocalDamage(previousSources, state.commanderSources); remapLocalCastCounts(previousSources, state.commanderSources); state.selectedSourceId = null; render(); return; }
  try { dom.saveCommanderCountButton.disabled = true; const result = await transport.setCommanderSetup(count, names); if (result.conflict) showError(new Error('The table changed first. The latest commander setup is shown.')); } catch (error) { showError(error); } finally { dom.saveCommanderCountButton.disabled = false; }
}
...
$('#createForm').addEventListener('submit', async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const playerCount = Number(form.get('playerCount')); const ownerCommanderCount = Number(form.get('commanderCount')); const ownerCommanderNames = commanderNamesFromForm(form, ownerCommanderCount); const ownerPlayerId = dom.ownerSeat.value; const ownerName = String(form.get('name') || '').trim() || ownerPlayerId; const enteredRoundLimit = String(form.get('roundLimitMinutes') || '').trim(); const roundLimitMinutes = enteredRoundLimit ? Number(enteredRoundLimit) : null; if (roundLimitMinutes !== null && (!Number.isInteger(roundLimitMinutes) || roundLimitMinutes < 1 || roundLimitMinutes > 999)) { dom.roundLimitMinutes.focus(); return; } const config = { playerCount, startingLife: Number(form.get('startingLife')), ownerPlayerId, ownerName, ownerCommanderCount, ownerCommanderNames, roundLimitMinutes }; if (dom.localSimulation.checked) return beginLocalGame({ ...config, localSimulation: true, podCode: 'LOCAL' }); try { const result = await transport.createRoom({ playerCount, startingLife: config.startingLife, commanderCount: ownerCommanderCount, commanderNames: ownerCommanderNames, name: ownerName, roundLimitMinutes }); showSharedGame(result.snapshot); } catch (error) { showError(error); } });
$('#joinForm').addEventListener('submit', async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const code = $('#podCode').value.trim().toUpperCase(); const ownerPlayerId = $('#joinSeat').value; const seatId = Number(ownerPlayerId.slice(1)) - 1; const commanderCount = Number(form.get('joinCommanderCount')); const commanderNames = commanderNamesFromForm(form, commanderCount); const ownerName = String(form.get('name') || '').trim() || ownerPlayerId; try { const room = await transport.inspectRoom(code); if (!room.snapshot.seats[seatId]) throw new Error('That seat does not exist in this pod.'); const result = await transport.claimRoom({ code, seatId, name: ownerName, commanderCount, commanderNames }); showSharedGame(result.snapshot); } catch (error) { showError(error); } });
...
dom.commanderCountForm.addEventListener('submit', event => { if (event.submitter?.value === 'confirm') { const form = new FormData(dom.commanderCountForm); const count = Number(form.get('gameCommanderCount')); updateCommanderSetup(count, commanderNamesFromForm(form, count)); } }); dom.declareWinnerForm.addEventListener('submit', event => { if (event.submitter?.value === 'confirm') declareWinner(); }); dom.connectionButton.addEventListener('click', () => dom.connectionDialog.showModal());
