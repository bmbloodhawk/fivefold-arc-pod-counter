...
async function showMyGames(signIn = false) {
  ...
  try {
    ...
    myGamesContent.innerHTML = history.recentGames.length ? `<ul>${history.recentGames.map(game => `<li><strong>${game.won ? 'Win' : 'Game'} · ${game.tableSize} players</strong><br><small>${new Date(game.savedAt).toLocaleDateString()}${game.commanderName ? ` · ${escapeHtml(game.commanderName)}` : ''}</small></li>`).join('')}</ul>` : '<p>No saved games yet.</p>'; myGamesContent.hidden = false;
...
async function showMyDecks(signIn = false) {
  ...
  try {
    ...
    const decks = (await response.json()).decks; myDecksStatus.textContent = decks.length ? `${decks.length} saved deck${decks.length === 1 ? '' : 's'}` : 'No saved decks yet.'; myDecksContent.innerHTML = decks.length ? `<ul>${decks.map(deck => `<li><strong>${escapeHtml(deck.commanderName)}</strong>${deck.name ? `<br><small>${escapeHtml(deck.name)}</small>` : ''}</li>`).join('')}</ul>` : ''; myDecksContent.hidden = !decks.length; myDecksSignInButton.hidden = true; createDeckButton.disabled = false;
