...
function closeGameMenu() { dom.gameMenu.hidden = true; dom.moreButton.setAttribute('aria-expanded', 'false'); }
document.addEventListener('pointerdown', event => { if (dom.gameMenu.hidden || dom.gameMenu.contains(event.target) || dom.moreButton.contains(event.target)) return; closeGameMenu(); });
dom.moreButton.addEventListener('click', () => { dom.gameMenu.hidden = !dom.gameMenu.hidden; dom.moreButton.setAttribute('aria-expanded', String(!dom.gameMenu.hidden)); }); dom.coinTossButton.addEventListener('click', () => { closeGameMenu(); tossCoin(); }); dom.declareWinnerButton.addEventListener('click', openDeclareWinner); dom.tossAgainButton.addEventListener('click', () => tossCoin()); dom.saveGameButton.addEventListener('click', saveGameToHistory); myGamesButton.addEventListener('click', openMyProfile); myGamesSignInButton.addEventListener('click', async () => { try { myGamesStatus.textContent = 'Signing in…'; await finishAccountSignIn(await googleAccountToken()); } catch (error) { myGamesStatus.textContent = error?.message || 'Sign-in did not work.'; } }); $('#resetButton').addEventListener('click', () => { closeGameMenu(); openResetDialog(); }); dom.confirmResetButton.addEventListener('click', resetGame);
