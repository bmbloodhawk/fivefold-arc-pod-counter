...
function selectSetupDeck(event) { selectedDeckId = event.currentTarget.value || ''; try { sessionStorage.setItem('fivefold-arc:selected-deck', selectedDeckId); } catch {} if (event.currentTarget === dom.createDeck && selectedDeckId) { const deck = setupDecks.find(item => item.deckId === selectedDeckId); const commanderNames = deck?.commanderNames?.length ? deck.commanderNames : [deck?.commanderName].filter(Boolean); if (commanderNames.length) { $(`input[name="commanderCount"][value="${commanderNames.length}"]`).checked = true; renderCommanderNameFields(dom.createCommanderNames, commanderNames.length, commanderNames); } } }
