...
async function refreshJoinSeats({ reveal = false } = {}) {
  ...
  try {
    ...
    if (reveal && openSeats.length) dom.joinDetails.hidden = false;
...
$('#createPodButton').addEventListener('click', () => showView(dom.create)); $('#joinPodButton').addEventListener('click', () => { showView(dom.join); dom.joinDetails.hidden = true; dom.joinCodeStatus.textContent = ''; }); $$('[data-back]').forEach(button => button.addEventListener('click', () => showView(dom.landing))); dom.playerCountChoices.addEventListener('change', event => syncOwnerChoices(Number(event.target.value))); dom.ownerSeat.addEventListener('change', () => { dom.createName.placeholder = dom.ownerSeat.value; }); $('#joinSeat').addEventListener('change', () => { dom.joinName.placeholder = dom.joinSeat.value; }); dom.joinCodeForm.addEventListener('submit', async event => { event.preventDefault(); await refreshJoinSeats({ reveal: true }); }); $$('input[name="commanderCount"], input[name="joinCommanderCount"]').forEach(input => input.addEventListener('change', refreshSetupCommanderNames));
