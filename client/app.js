...
async function showMyGames(signIn = false) {
  ...
  try {
    ...
    const history = (await response.json()).history; exportAccountButton.hidden = false; deleteAccountButton.hidden = false; myGamesStatus.textContent = `${history.wins} wins in ${history.gamesPlayed} games${history.gamesPlayed ? ` · ${Math.round(history.winRate * 100)}% win rate` : ''}`;
