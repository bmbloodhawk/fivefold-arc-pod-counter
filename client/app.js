...
function showAchievementUnlocks(gameKey) {
  ...
  const [achievement, ...more] = unlocked;
  const rarity = achievement.rarity || 'common';
  dom.achievementDialog.dataset.rarity = rarity; dom.achievementDialog.dataset.gameKey = gameKey; dom.achievementEmblem.textContent = achievementRarityMark(rarity);
  dom.achievementRarity.textContent = achievementRarityLabel(rarity); dom.achievementTitle.textContent = achievement.title; dom.achievementDetail.textContent = achievement.detail;
  dom.achievementMore.hidden = more.length === 0; dom.achievementMore.textContent = more.length ? `Also uncovered: ${more.map(item => item.title).join(' · ')}` : '';
