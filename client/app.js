...
let selectedDeckId = sessionStorage.getItem('fivefold-arc:selected-deck') || '';
...
async function loadSetupDecks() {
  try { const token = await currentAccountToken(); if (!token) return; const response = await fetch('/api/account/decks', { headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return; const { decks } = await response.json(); const options = `<option value="">No deck selected</option>${decks.map(deck => `<option value="${escapeHtml(deck.deckId)}">${escapeHtml([deck.commanderName, deck.name].filter(Boolean).join(' · '))}</option>`).join('')}`; [dom.createDeck, dom.joinDeck].forEach(select => { select.innerHTML = options; select.value = selectedDeckId; }); dom.createDeckField.hidden = false; dom.joinDeckField.hidden = false; } catch {} }
...
async function createDeck() {
  const token = await currentAccountToken(); if (!token || !deckCommanderName.value.trim()) return;
  createDeckButton.disabled = true;
  try { const response = await fetch('/api/account/decks', { method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ commanderName: deckCommanderName.value, name: deckName.value }) }); if (!response.ok) throw new Error('This deck could not be saved yet.'); deckCommanderName.value = ''; deckName.value = ''; await showMyDecks(); } catch (error) { myDecksStatus.textContent = error?.message || 'This deck could not be saved yet.'; createDeckButton.disabled = false; }
}
...
function selectSetupDeck(event) { selectedDeckId = event.currentTarget.value || ''; try { sessionStorage.setItem('fivefold-arc:selected-deck', selectedDeckId); } catch {} }
