...
import { currentAccountToken, emailAccountToken, googleAccountToken } from './account-auth.js?v=3';
...
emailSignInButton.addEventListener('click', async () => { emailSignInFields.hidden = false; createAccountButton.hidden = false; if (!accountEmail.value || !accountPassword.value) return; try { await emailAccountToken(accountEmail.value, accountPassword.value); showMyGames(); } catch (error) { myGamesStatus.textContent = error?.message || 'Email sign-in did not work.'; } });
