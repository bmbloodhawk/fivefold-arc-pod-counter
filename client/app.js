...
let selectedDeckId = sessionStorage.getItem('fivefold-arc:selected-deck') || ''; let setupDecks = [];
...
async function refreshJoinSeats() {
  ...
  try {
    ...
    if (availableSeats.length) { void loadSetupDecks(); showJoinStep(2); showView(dom.joinSeatView); }
...
emailSignInButton.addEventListener('click', async () => { emailSignInFields.hidden = false; forgotPasswordButton.hidden = false; createAccountButton.hidden = false; if (!accountEmail.value || !accountPassword.value) return; try { await emailAccountToken(accountEmail.value, accountPassword.value); showMyGames(); } catch (error) { myGamesStatus.textContent = error?.message || 'Email sign-in did not work.'; } });
